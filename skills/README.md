...
Skills are specialized prompts that enhance OmniAgent with domain-specific knowledge and task-specific workflows. They help developers by providing expert guidance, automating common tasks, and ensuring consistent practices across projects. Each skill is designed to excel in a specific area, from Git operations to code review processes.

## Terminology Note

**Version 0 (V0)**: The term "microagents" continues to be used for V0 conversations. V0 is the current stable version of OmniAgent.
...
This directory (`OmniAgent/skills/`) contains shareable skills that will be used in V1 conversations. For V0 conversations, the system continues to use microagents from the same underlying files.

## Sources of Skills/Microagents

OmniAgent loads skills (V1) or microagents (V0) from two sources:

### 1. Shareable Skills/Microagents (Public)

This directory (`OmniAgent/skills/`) contains shareable skills (V1) or microagents (V0) that are:

- Available to all OmniAgent users
- Maintained in the OmniAgent repository
...
Each repository can have its own instructions in `.omniagent/microagents/` (V0) or `.omniagent/skills/` (V1). These instructions are:
...
- V1 supports both `.omniagent/skills/` (preferred) and `.omniagent/microagents/` (backward compatibility)
...
When OmniAgent works with a repository, it:

1. Loads repository-specific instructions from `.omniagent/microagents/repo.md` (V0) or `.omniagent/skills/` (V1) if present
2. Loads relevant knowledge agents based on keywords in conversations

**Note**: V1 also supports loading from `.omniagent/microagents/` for backward compatibility.
...
You can see an example of a knowledge-based agent in [OmniAgent's github skill](https://github.com/ismailubts/OmniAgent/tree/main/skills/github.md).

### 2. Repository Agents

Repository agents provide repository-specific knowledge and guidelines. They are:

- Loaded from `.omniagent/microagents/repo.md` (V0) or `.omniagent/skills/` directory (V1)
- V1 also supports `.omniagent/microagents/` for backward compatibility
...
You can see an example of a repo agent in [the glossary for the OmniAgent repo](https://github.com/ismailubts/OmniAgent/blob/main/.omniagent/microagents/glossary.md).
...
2. **For Repository Agents**:
   ...
   - Consider using OmniAgent to generate a comprehensive repo.md (see [Creating a Repository Agent](#creating-a-repository-agent))
...
1. Create your agent file in the appropriate directory:
   ...
   - Note: Repository-specific agents should remain in their respective repositories' `.omniagent/skills/` (V1) or `.omniagent/microagents/` (V0) directory
2. Test thoroughly
3. Submit a pull request to OmniAgent

## License

All skills/microagents are subject to the same license as OmniAgent. See the root LICENSE file for details.
