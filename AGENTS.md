...
(in the `omniagent` directory) and React frontend (in the `frontend` directory).

## General Setup:
To set up the entire repo, including frontend and backend, run `make build`.
You don't need to do this unless the user asks you to, or if you're trying to run the entire application.

## Running OmniAgent locally:
To run the full application to debug issues:
```bash
export INSTALL_DOCKER=0
export RUNTIME=local
make build && make run FRONTEND_PORT=12000 FRONTEND_HOST=0.0.0.0 BACKEND_HOST=0.0.0.0 &> /tmp/omniagent-log.txt &
```
...
* If you've made changes to the VSCode extension, you should run `cd omniagent/app_server/integrations/vscode && npm run lint:fix && npm run compile ; cd ../../..`
...
- GitHub-authored (`actions/*`, `github/*`) and first-party (`OmniAgent/*`) actions are currently exempt.
...
- The current V1 application server lives in `omniagent/app_server/`. `make start-backend` still launches `omniagent.server.listen:app`, which includes the V1 routes by default unless `ENABLE_V1=0`.
...
VSCode Extension:
- Located in the `omniagent/app_server/integrations/vscode` directory
...
The `enterprise/` directory contains additional functionality that extends the open-source OmniAgent codebase. This includes:
...
**Setup Steps:**
1. First, build the main OmniAgent project: `make build`
...
- When the `omniagent` package (root project) version has been updated, run `poetry lock` in the `enterprise/` folder to update the version in the enterprise poetry lockfile.
...
- This ensures code works in both OmniAgent and enterprise contexts
...
- For frontend issues, ensure the main OmniAgent frontend is built: `make build`
...
Microagents are specialized prompts that enhance OmniAgent with domain-specific knowledge and task-specific workflows. They are Markdown files that can include frontmatter for configuration.

#### Types:
- **Public Microagents**: Located in `microagents/`, available to all users
- **Repository Microagents**: Located in `.omniagent/microagents/`, specific to this repository
...
- To add a new user setting to OmniAgent, follow these steps:
  ...
  2. Add the setting to the backend:
     - Add the setting to the `Settings` model in `omniagent/app_server/settings/settings_models.py`
     - Update any relevant backend code to apply the setting (e.g., in session creation)

#### Settings UI Patterns:

There are two main patterns for saving settings in the OmniAgent frontend:
...
To add a new LLM model to OmniAgent, you need to update multiple files across both frontend and backend:
...
1. **Frontend Model Arrays** (`frontend/src/utils/verified-models.ts`):
   ...
   - Add to provider-specific arrays based on the model's provider:
     ...
     - `VERIFIED_OMNIAGENT_MODELS` for models available through OmniAgent provider

2. **Backend CLI Integration** (`omniagent/cli/utils.py`):
   - Add the model to the appropriate `VERIFIED_*_MODELS` arrays
   - This ensures the model appears in CLI model selection

3. **Backend Model List** (`omniagent/utils/llm.py`):
   - **CRITICAL**: Add the model to the `openhands_models` list (lines 57-66) if using OmniAgent provider
   - This is required for the model to appear in the frontend model selector
   - Format: `'omniagent/model-name'` (e.g., `'openhands/o3'`)

4. **Backend LLM Configuration** (`omniagent/llm/llm.py`):
...
- **VERIFIED_OMNIAGENT_MODELS**: Models available through OmniAgent managed provider
...
#### User Credentials with Exposed Secrets (in `omniagent/app_server/user/user_router.py`):
...
- Bearer token (proves user identity via `OMNIAGENT_API_KEY`)
...
#### Sandbox-Scoped Secrets Endpoints (in `omniagent/app_server/sandbox/sandbox_router.py`):
...
- `omniagent/sdk/llm/llm.py`: `LLM.api_key` accepts `SecretSource` (including `LookupSecret`)
- `omniagent/workspace/cloud/workspace.py`: `get_llm()` and `get_secrets()` return LookupSecret-backed objects
...
- The OmniAgent classifier logic for newcomer suitability lives in `scripts/issue_good_first_issue_check_openhands.py`, with focused unit coverage in `tests/unit/test_issue_good_first_issue_check_openhands.py`.
