...
from omniagent.app_server.app import app
from omniagent.app_server.file_store.memory import InMemoryFileStore
from omniagent.app_server.integrations.provider import ProviderToken, ProviderType
from omniagent.app_server.integrations.service_types import UserGitInfo
from omniagent.app_server.secrets.secrets_models import Secrets
from omniagent.app_server.secrets.secrets_store import SecretsStore
from omniagent.app_server.settings.file_settings_store import FileSettingsStore
from omniagent.app_server.settings.settings_models import MarketplaceRegistration
from omniagent.app_server.settings.settings_store import SettingsStore
from omniagent.app_server.user.skills_router import (
    GLOBAL_SKILLS_DIR,
...
def test_client():
    with (
        ...
        patch('omniagent.app_server.utils.dependencies._SESSION_API_KEY', None),
        patch(
            'omniagent.app_server.user_auth.user_auth.UserAuth.get_instance',
            return_value=MockUserAuth(),
        ),
        patch(
            'omniagent.app_server.settings.file_settings_store.FileSettingsStore.get_instance',
...
async def test_skills_search_returns_skills(test_client, tmp_path):
    ...
    with (
        patch('omniagent.app_server.user.skills_router.GLOBAL_SKILLS_DIR', global_dir),
        patch(
            'omniagent.app_server.user.skills_router.USER_SKILLS_DIR',
...
async def test_skills_search_handles_missing_dirs(test_client, tmp_path):
    ...
    with (
        patch(
            'omniagent.app_server.user.skills_router.GLOBAL_SKILLS_DIR',
...
async def test_skills_search_sorted_by_source_then_name(test_client, tmp_path):
    ...
    with (
        patch('omniagent.app_server.user.skills_router.GLOBAL_SKILLS_DIR', global_dir),
        patch('omniagent.app_server.user.skills_router.USER_SKILLS_DIR', user_dir),
    ):
        response = test_client.get('/api/v1/skills/search')
...
class TestMarketplaceSkillsCloneFailures:
    ...
    async def test_clone_invalid_repo_path(self):
        ...
        with patch(
            'omniagent.app_server.user.skills_router._parse_marketplace_source',
            return_value=('github', ''),
        ):
    ...
    async def test_clone_network_timeout(self):
        ...
        with patch(
            'omniagent.app_server.user.skills_router.subprocess.run'
        ) as mock_run:
    ...
    async def test_clone_with_ref_checkout_failure(self):
        ...
        with patch(
            ...
            with patch('omniagent.app_server.user.skills_router._cleanup_clone_dir'):
    ...
    async def test_clone_repo_path_not_found(self):
        ...
        with patch(
            ...
            with patch(
                'omniagent.app_server.user.skills_router.Path.exists'
            ) as mock_exists:
                with patch(
                    'omniagent.app_server.user.skills_router._cleanup_clone_dir'
                ):
                    with patch(
