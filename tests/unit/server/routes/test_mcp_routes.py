...
from omniagent.app_server.integrations.service_types import GitService
from omniagent.app_server.mcp.mcp_router import get_conversation_link, init_tavily_proxy
...
def test_mcp_server_no_stateless_http_deprecation_warning():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        from omniagent.app_server.mcp.mcp_router import mcp_server
...
async def test_get_conversation_link_non_saas_mode():
    ...
    with patch('omniagent.app_server.mcp.mcp_router.get_global_config') as mock_config:
...
async def test_get_conversation_link_saas_mode():
    ...
    # Test with SAAS mode
    with (
        patch('omniagent.app_server.mcp.mcp_router.get_global_config') as mock_config,
        patch(
            'omniagent.app_server.mcp.mcp_router.CONVERSATION_URL',
...
class TestInitTavilyProxy:
    ...
    def test_init_tavily_proxy_no_api_key(self):
        ...
        with (
            patch(
                'omniagent.app_server.mcp.mcp_router.get_global_config'
            ) as mock_config,
            patch('omniagent.app_server.mcp.mcp_router.logger') as mock_logger,
            patch('omniagent.app_server.mcp.mcp_router.Client') as mock_client,
            patch('omniagent.app_server.mcp.mcp_router.create_proxy') as mock_proxy,
            patch('omniagent.app_server.mcp.mcp_router.mcp_server') as mock_mcp_server,
        ):
    ...
    def test_init_tavily_proxy_with_api_key(self):
        ...
        with (
            ...
            patch('omniagent.app_server.mcp.mcp_router.Client') as mock_client,
            patch(
                'omniagent.app_server.mcp.mcp_router.StreamableHttpTransport'
            ) as mock_transport,
            patch(
                'omniagent.app_server.mcp.mcp_router.create_proxy'
    ...
    def test_init_tavily_proxy_handles_exception(self):
        ...
        with (
            ...
            patch('omniagent.app_server.mcp.mcp_router.logger') as mock_logger,
            patch('omniagent.app_server.mcp.mcp_router.Client') as mock_client,
            patch('omniagent.app_server.mcp.mcp_router.StreamableHttpTransport'),
            patch('omniagent.app_server.mcp.mcp_router.mcp_server') as mock_mcp_server,
        ):
            # Configure API key
            mock_config.return_value.tavily_api_key = 'test-tavily-key'
