...
import omniagent.app_server.settings.settings_models as settings_module
from omniagent.app_server.settings.llm_profiles import ProfileNotFoundError
...
def test_litellm_proxy_custom_endpoint_keeps_prefix():
    ...
    # Display should NOT convert to omniagent/ because it's a custom endpoint
    api_data = settings.get_agent_settings_display()
...
class TestMarketplaceRegistration:
    ...
    def test_repo_path_validation_rejects_url(self):
        ...
        with pytest.raises(ValidationError, match='not a URL'):
            MarketplaceRegistration(
                name='test',
                source='github:OmniAgent/extensions',
                repo_path='https://github.com/OmniAgent/extensions',
            )
