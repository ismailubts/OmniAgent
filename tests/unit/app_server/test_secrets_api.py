...
from omniagent.app_server.secrets.file_secrets_store import FileSecretsStore
...
def test_client():
    ...
    with patch.dict(os.environ, {'SESSION_API_KEY': ''}, clear=False):
        ...
        with patch('omniagent.app_server.utils.dependencies._SESSION_API_KEY', None):
...
def file_secrets_store(temp_dir):
    ...
    with patch(
        'omniagent.app_server.secrets.file_secrets_store.FileSecretsStore.get_instance',
...
async def test_add_git_providers_with_host(test_client, file_secrets_store):
    ...
    with patch(
        'omniagent.app_server.secrets.secrets_router.check_provider_tokens',
...
async def test_add_git_providers_invalid_token_with_host(
    ...
    with patch(
        'omniagent.app_server.integrations.utils.validate_provider_token',
