...
class TestProcessStatsEvent:
    ...
    async def test_process_stats_event_service_error_handled(
        ...
        with (
            ...
            patch(
                'omniagent.app_server.app_conversation.sql_app_conversation_info_service.logger'
            ) as mock_logger,
        ):
            await service.process_stats_event(
...
class TestOnEventStatsProcessing:
    ...
    async def test_on_event_processes_stats_events(self):
        ...
        from omniagent.app_server.event_callback.webhook_router import on_event
        ...
        with patch(
            'omniagent.app_server.event_callback.webhook_router._run_callbacks_in_bg_and_close'
        ) as mock_callbacks:
