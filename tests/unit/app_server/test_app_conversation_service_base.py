...
from omniagent.app_server.app_conversation.app_conversation_models import AgentType
...
from omniagent.app_server.sandbox.sandbox_models import SandboxInfo, SandboxStatus
...
@patch(
    'omniagent.app_server.app_conversation.app_conversation_service_base.LLMSummarizingCondenser'
)
...
def test_create_security_analyzer_logs_warning_for_unknown_value():
    ...
    # Act
    with patch(
        'omniagent.app_server.app_conversation.app_conversation_service_base._logger'
    ) as mock_logger:
...
async def test_azure_devops_git_credential_helper_logs_without_web_url(
    ...
    with patch(
        'omniagent.app_server.app_conversation.app_conversation_service_base._logger.debug'
    ) as mock_debug:
...
class TestLoadAndMergeAllSkills:
    ...
    @pytest.mark.asyncio
    @patch(
        'omniagent.app_server.app_conversation.app_conversation_service_base.load_skills_from_agent_server'
    )
    @patch(
        'omniagent.app_server.app_conversation.app_conversation_service_base.build_org_configs'
    )
    @patch(
        'omniagent.app_server.app_conversation.app_conversation_service_base.build_sandbox_config'
    )
    async def test_loads_skills_successfully(
        ...
        with patch.object(AppConversationServiceBase, '__abstractmethods__', set()):
            ...
            from omniagent.app_server.sandbox.sandbox_models import ExposedUrl
