...
from omniagent.app_server.integrations.provider import ProviderHandler, ProviderToken
...
from omniagent.app_server.sandbox.sandbox_router import (
    get_secret_value,
    list_secret_names,
)
from omniagent.app_server.sandbox.session_auth import (
...
from omniagent.app_server.user.user_router import get_current_user
...
def _patch_sandbox_service(return_sandbox: SandboxInfo | None):
    ...
    ctx = patch(
        'omniagent.app_server.sandbox.session_auth.get_sandbox_service',
    )
...
class TestValidateSessionKey:
    ...
    async def test_rejects_sandbox_without_user_in_saas_mode(self):
        ...
        with (
            ctx as mock_get,
            patch(
                'omniagent.app_server.sandbox.session_auth.get_global_config'
            ) as mock_cfg,
        ):
            mock_get.return_value.__aenter__ = AsyncMock(return_value=mock_svc)
            mock_get.return_value.__aexit__ = AsyncMock(return_value=False)

            from omniagent.app_server.types import AppMode
...
class TestGetCurrentUserExposeSecrets:
    ...
    async def test_expose_secrets_returns_raw_api_key(self):
        ...
        with patch(
            'omniagent.app_server.user.user_router.validate_session_key_ownership'
        ) as mock_validate:
...
class TestListSecretNames:
    ...
    async def test_returns_secret_names_without_values(self):
        ...
        with patch(
            'omniagent.app_server.sandbox.sandbox_router._get_user_context'
        ) as mock_ctx:
            ctx = AsyncMock()
...
def _build_integration_test_app(
    ...
    from omniagent.app_server.user.user_router import router as user_router
    from omniagent.app_server.utils.dependencies import check_session_api_key
    ...
    if mock_user_context is not None:
        from omniagent.app_server.user.user_router import user_dependency
...
class TestProviderTokensInEndpoints:
    ...
    async def test_get_provider_tokens_as_env_vars_continues_after_refresh_error(
        ...
        with patch(
            'omniagent.app_server.user.auth_user_context._logger.warning'
        ) as mock_warning:
            result = await ctx.get_provider_tokens(as_env_vars=True)
