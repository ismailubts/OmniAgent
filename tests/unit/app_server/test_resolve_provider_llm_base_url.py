"""Tests for resolve_provider_llm_base_url in omniagent.app_server.config."""
...
SDK_DEFAULT = _SDK_DEFAULT_PROXY  # 'https://llm-proxy.app.github.com/ismailubts/OmniAgent/'
STAGING_URL = 'https://llm-proxy.staging.github.com/ismailubts/OmniAgent/'
CUSTOM_URL = 'https://my-own-proxy.example.com/v1'


class TestOmniAgentPrefixWithProviderUrl:
    """omniagent/ prefix + SDK default URL + provider URL set → returns provider URL."""

    def test_openhands_prefix_replaces_sdk_default(self):
        result = resolve_provider_llm_base_url(
            model='omniagent/gpt-4',
...
class TestOmniAgentPrefixWithCustomUrl:
    """omniagent/ prefix + custom URL (not SDK default) → returns custom URL."""
...
class TestOmniAgentPrefixNoProviderUrl:
    """omniagent/ prefix + SDK default URL + no provider URL → returns SDK default."""
    ...
    def test_sdk_default_returned_when_provider_none_and_env_unset(self, monkeypatch):
        monkeypatch.delenv('OMNIAGENT_PROVIDER_BASE_URL', raising=False)
...
class TestNonMatchingModelPrefix:
    """Model without omniagent/ prefix → returns base_url unchanged."""
...
class TestEdgeCases:
    ...
    def test_env_fallback_when_provider_base_url_is_none(self, monkeypatch):
        monkeypatch.setenv('OMNIAGENT_PROVIDER_BASE_URL', STAGING_URL)
        result = resolve_provider_llm_base_url(
            model='omniagent/gpt-4',
            base_url=SDK_DEFAULT,
