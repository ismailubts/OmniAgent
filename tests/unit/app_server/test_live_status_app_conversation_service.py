...
from omniagent.app_server.app_conversation.live_status_app_conversation_service import (
    LiveStatusAppConversationService,
    _merge_launch_context,
    effective_disabled_skills,
)
from omniagent.app_server.integrations.provider import ProviderToken, ProviderType
from omniagent.app_server.integrations.service_types import SuggestedTask, TaskType
...
from omniagent.app_server.sandbox.sandbox_spec_models import SandboxSpecInfo
from omniagent.app_server.settings.llm_profiles import LLMProfiles
...
from omniagent.app_server.utils.redis_lock import RedisLockUnavailable
from openhands.sdk import Agent, AgentContext, Event
from openhands.sdk.llm import LLM
from openhands.sdk.profiles import OmniAgentAgentProfile, resolve_agent_profile
...
class TestLiveStatusAppConversationService:
    ...
    @pytest.mark.parametrize(
        'llm_base_url',
        [
            'https://llm-proxy.app.github.com/ismailubts/OmniAgent',
            'https://llm-proxy.staging.github.com/ismailubts/OmniAgent',
            None,
        ],
    )
    ...
    async def test_maybe_refresh_managed_llm_key_skips_non_saas(self, monkeypatch):
        ...
        llm = LLM(
            model='omniagent/gpt-5.5',
            base_url='https://llm-proxy.app.github.com/ismailubts/OmniAgent',
            api_key=SecretStr('sk-old-managed-key'),
        )
    ...
    async def test_configure_llm_and_mcp_with_custom_model(self):
        ...
        assert default_server.headers[
            'X-OmniAgent-ServerConversation-ID'
    ...
    async def test_configure_llm_and_mcp_openhands_model_uses_user_base_url(
        ...
        """omniagent/* model uses user's base_url when set."""
        # Arrange
        self.mock_user.llm_model = 'omniagent/special'
        ...
        # Assert — user base_url takes precedence for omniagent/ models
    ...
    async def test_configure_llm_and_mcp_openhands_model_falls_back_to_provider_url(
        ...
        """omniagent/* model falls back to provider base URL when user has no base_url."""
        # Arrange
        self.mock_user.llm_model = 'omniagent/default'
    ...
    async def test_configure_llm_and_mcp_openhands_model_no_base_urls(self):
        """omniagent/* model is kept public when no explicit base URL exists."""
        # Arrange
        self.mock_user.llm_model = 'omniagent/default'
        self.mock_user.llm_base_url = None
        self.service.omniagent_provider_base_url = None
        self.mock_user_context.get_mcp_api_key.return_value = None
        ...
        # Assert
        assert llm.model == 'omniagent/default'
    ...
    async def test_configure_llm_and_mcp_litellm_proxy_model_keeps_empty_base_url(
        ...
        """litellm_proxy/* is not treated as an OmniAgent provider model."""
        self.mock_user.llm_base_url = None
        self.mock_user_context.get_mcp_api_key.return_value = None
    ...
    async def test_configure_llm_and_mcp_non_openhands_model_ignores_provider(self):
        ...
        self.mock_user.llm_base_url = 'https://user-llm.example.com'
        self.service.omniagent_provider_base_url = 'https://provider.example.com'
        self.mock_user_context.get_mcp_api_key.return_value = None
    ...
    def test_build_observability_context_includes_repository(self):
        ...
        metadata, tags = self.service._build_observability_context(
            ...
            selected_repository='OmniAgent/software-agent-sdk',
        ...
        assert metadata == {
            ...
            'agent_kind': 'openhands',
            'repo_name': 'OmniAgent/software-agent-sdk',
            'selected_branch': 'main',
            'git_provider': 'github',
        }
        assert 'repo:OmniAgent/software-agent-sdk' in tags
        assert 'branch:main' in tags
        assert 'git_provider:github' in tags

    def test_apply_server_overrides_adds_repo_metadata(self):
        llm = LLM(model='omniagent/gpt-4', api_key='k', usage_id='agent')
        ...
        assert metadata['repo_name'] == 'OmniAgent/software-agent-sdk'
        assert metadata['git_provider'] == 'github'
        assert metadata['selected_branch'] == 'main'
        assert 'repo:OmniAgent/software-agent-sdk' in metadata['tags']
        assert 'branch:main' in metadata['tags']
        assert 'git_provider:github' in metadata['tags']

    @patch(
        'omniagent.app_server.app_conversation.live_status_app_conversation_service.get_default_tools',
        return_value=[],
    )
    @pytest.mark.asyncio
    ...
    async def test_build_request_skills_loading_fails_gracefully(self, _mock_tools):
        ...
        with patch(
            'omniagent.app_server.app_conversation.live_status_app_conversation_service._logger'
        ) as mock_logger:
            result = await self.service._build_start_conversation_request_for_user(
                sandbox=self.mock_sandbox,
                conversation_id=conversation_id,
    ...
    async def test_planning_launch_uses_planning_tools_over_profile_tools(self):
        ...
        with (
            ...
            patch(
                'omniagent.app_server.app_conversation.live_status_app_conversation_service.get_planning_tools',
    ...
    @patch(
        'omniagent.app_server.app_conversation.live_status_app_conversation_service.get_registered_agent_definitions'
    )
    @patch(
        'omniagent.app_server.app_conversation.live_status_app_conversation_service.register_builtins_agents'
    )
    @patch(
        'omniagent.app_server.app_conversation.live_status_app_conversation_service.get_default_tools',
        return_value=[],
    ...
    async def test_open_conversation_export_streams_with_lock(self):
        ...
        with patch(
            'omniagent.app_server.app_conversation.live_status_app_conversation_service.try_acquire_redis_lock',
    ...
    @patch(
        'omniagent.app_server.app_conversation.live_status_app_conversation_service.AsyncRemoteWorkspace'
    )
    @patch(
        'omniagent.app_server.app_conversation.live_status_app_conversation_service.ConversationInfo'
    )
    ...
    async def test_start_app_conversation_preserves_acp_and_repository_tags(
        ...
        request = AppConversationStartRequest(
            selected_repository='ismailubts/OmniAgent',
            selected_branch='main',
            git_provider=ProviderType.GITHUB,
        ...
        assert saved_info.tags['repo_name'] == 'ismailubts/OmniAgent'
    ...
    def test_get_project_dir_with_repo(self):
        ...
        assert (
            get_project_dir('/workspace/project', 'OmniAgent/software-agent-sdk')
    ...
    async def test_build_request_workspace_uses_project_dir(self, _mock_tools):
        ...
        .omniagent/hooks/on_stop.sh is not found.
        """
        self.mock_user_context.get_user_info.return_value = self.mock_user
...
class TestBuildAcpStartConversationRequestSecrets:
    ...
    async def test_observability_metadata_populated(self, service, tmp_path):
        """Repo / branch / provider land on the request's observability_metadata
        for ACP conversations too, matching the OmniAgent-agent path."""
        user = self._make_acp_user()
    ...
    async def test_observability_metadata_includes_commit(self, service, tmp_path):
        """The post-clone HEAD sha is resolved from the workspace and added,
        mirroring the OmniAgent-agent path's commit resolution."""
        user = self._make_acp_user()
        remote_workspace = Mock(spec=AsyncRemoteWorkspace)
        remote_workspace.execute_command = AsyncMock(
            return_value=SimpleNamespace(exit_code=0, stdout='abc123sha\n')
