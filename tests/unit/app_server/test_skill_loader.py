...
from omniagent.app_server.app_conversation.skill_loader import (
    _MAX_ORG_CANDIDATES,
    OrgConfig,
    SandboxConfig,
    SkillInfo,
...
from omniagent.app_server.integrations.provider import ProviderType
from omniagent.app_server.integrations.service_types import AuthenticationError
...
class TestGetProviderType:
    ...
    @pytest.mark.asyncio
    @patch('omniagent.app_server.app_conversation.skill_loader._is_gitlab_repository')
    @patch(
        'omniagent.app_server.app_conversation.skill_loader._is_azure_devops_repository'
    )
...
class TestBuildOrgConfigs:
    ...
    @pytest.mark.asyncio
    @patch('omniagent.app_server.app_conversation.skill_loader._get_org_repository_url')
    async def test_loads_account_and_orgs_when_no_repository(
        self, mock_get_url, mock_user_context
    ):
        """Account and org repos (both .omniagent and .agents) load with no repo."""
        # Arrange
    ...
    @pytest.mark.asyncio
    @patch('omniagent.app_server.app_conversation.skill_loader._get_provider_type')
    @patch(
        'omniagent.app_server.app_conversation.skill_loader._determine_org_repo_path'
    )
    @patch('omniagent.app_server.app_conversation.skill_loader._get_org_repository_url')
    async def test_includes_selected_repo_owner_both_repos(
        ...
        """A selected GitHub repo loads the owner's .omniagent and .agents repos."""
        # Arrange
        handler = self._make_provider_handler(logins={})  # no global owners
        mock_user_context.get_provider_handler = AsyncMock(return_value=handler)
        mock_determine_path.return_value = ('other/.omniagent', 'other')
    ...
    async def test_dedupes_when_owner_equals_login(
        ...
        mock_user_context.get_provider_handler = AsyncMock(return_value=handler)
        mock_determine_path.return_value = ('hieptl/.omniagent', 'hieptl')
        mock_get_provider.return_value = 'github'
        mock_get_url.side_effect = lambda path, ctx: f'https://git/{path}.git'
        ...
        assert resolved_paths == ['hieptl/.agents', 'hieptl/.omniagent']

    @pytest.mark.asyncio
    @patch('omniagent.app_server.app_conversation.skill_loader._get_org_repository_url')
    async def test_skips_agents_for_gitlab(self, mock_get_url, mock_user_context):
        """GitLab owners resolve only omniagent-config (no .omniagent/.agents)."""
        # Arrange
        ...
        # Assert
        assert {c.repository for c in result} == {'mygroup/omniagent-config'}
    ...
    async def test_filters_unresolvable_repos(self, mock_get_url, mock_user_context):
        ...
        def _resolve(path, ctx):
            return f'https://git/{path}.git' if path == 'hieptl/.omniagent' else None
        ...
        # Assert
        assert {c.repository for c in result} == {'hieptl/.omniagent'}
    ...
    async def test_selected_repo_is_first_for_legacy_org_config(
        ...
        mock_user_context.get_provider_handler = AsyncMock(return_value=handler)
        mock_determine_path.return_value = ('selectedorg/.omniagent', 'selectedorg')
        mock_get_provider.return_value = 'github'
        mock_get_url.side_effect = lambda path, ctx: f'https://git/{path}.git'
        ...
        # Assert
        assert result[0].repository == 'selectedorg/.omniagent'
...
class TestLoadSkillsFromAgentServer:
    ...
    async def test_loads_skills_successfully(self, mock_client_class):
        ...
        org_config = OrgConfig(
            ...
            provider='github',
            org_repo_url='https://github.com/owner/.omniagent.git',
            org_name='owner',
        )
...
class TestDetermineOrgRepoPath:
    ...
    async def test_github_repository_path(self, mock_is_azure, mock_is_gitlab):
        ...
        # Assert
        assert org_repo == 'owner/.omniagent'
    ...
    async def test_gitlab_repository_path(self, mock_is_azure, mock_is_gitlab):
        ...
        # Assert
        assert org_repo == 'owner/omniagent-config'
        assert org_name == 'owner'
    ...
    async def test_azure_devops_repository_path(self, mock_is_azure, mock_is_gitlab):
        ...
        # Assert
        assert org_repo == 'org/omniagent-config/omniagent-config'
        assert org_name == 'org'
...
class TestGetOrgRepositoryUrl:
    ...
    async def test_successful_url_retrieval(self):
        ...
        # Arrange
        mock_user_context = AsyncMock()
        expected_url = 'https://token@github.com/owner/.omniagent.git'
        mock_user_context.get_authenticated_git_url.return_value = expected_url

        # Act
        result = await _get_org_repository_url('owner/.omniagent', mock_user_context)

        # Assert
        assert result == expected_url
        mock_user_context.get_authenticated_git_url.assert_called_once_with(
            'owner/.omniagent', is_optional=True
        )
...
class TestLoadSkillsWithMarketplaces:
    ...
    async def test_passes_registered_marketplaces_in_payload(self, mock_client_class):
        ...
        marketplaces = [
            MarketplaceRegistration(
                name='public',
                source='github:OmniAgent/skills',
                auto_load=True,
            ),
            MarketplaceRegistration(
                name='team',
        ...
        assert mp1['source'] == 'github:OmniAgent/skills'
...
AUTHENTICATED_URL = (
    'https://x-access-token:token123@github.com/OmniAgent/extensions-private.git'
)


def _make_registration(**overrides):
    """Build a MarketplaceRegistration with private-repo defaults."""
    from omniagent.app_server.settings.settings_models import MarketplaceRegistration

    fields = {
        'name': 'extensions-private',
        'source': 'github:OmniAgent/extensions-private',
        'auto_load': True,
    }
...
class TestAuthenticateMarketplaceSources:
    ...
    @pytest.mark.parametrize(
        'source',
        [
            'github:OmniAgent/extensions-private',
            'OmniAgent/extensions-private',
            'https://github.com/OmniAgent/extensions-private.git',
        ],
    )
    async def test_rewrites_auto_load_source_to_authenticated_url(self, source):
        ...
        mock_user_context.get_authenticated_git_url.assert_awaited_once_with(
            'OmniAgent/extensions-private', is_optional=True
        )
    ...
    async def test_returns_copy_preserving_fields_without_mutating_input(self):
        ...
        assert registration.source == 'github:OmniAgent/extensions-private'
    ...
    async def test_keeps_source_when_resolution_fails(self):
        ...
        # Assert
        assert result is not None
        assert result[0].source == 'github:OmniAgent/extensions-private'
