...
def reset_litellm():
    ...
    if 'omniagent.app_server.utils.logger' in sys.modules:
        del sys.modules['omniagent.app_server.utils.logger']
...
def test_litellm_settings_debug_llm_disabled(reset_litellm):
    ...
    with mock.patch.dict(os.environ, {'DEBUG_LLM': 'false'}):
        import omniagent.app_server.utils.logger  # noqa: F401

        importlib.reload(omniagent.app_server.utils.logger)
...
def test_litellm_loggers_suppressed_with_uvicorn_json_config(reset_litellm):
    ...
    # We need to go up to tests/, then find omniagent/app_server/utils/logger.py
