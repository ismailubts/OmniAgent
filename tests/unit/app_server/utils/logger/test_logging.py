...
from omniagent.app_server.utils.logger import (
    LOG_JSON_LEVEL_KEY,
    OmniAgentLoggerAdapter,
    json_log_handler,
)
from omniagent.app_server.utils.logger import omniagent_logger as omniagent_logger
...
def test_handler():
    ...
    omniagent_logger.addHandler(handler)
    yield omniagent_logger, stream
    omniagent_logger.removeHandler(handler)
...
def json_handler():
    ...
    omniagent_logger.addHandler(json_handler)
    yield omniagent_logger, stream
    omniagent_logger.removeHandler(json_handler)
...
def test_sensitive_env_vars_masking(test_handler):
    ...
    with patch.dict(
        'omniagent.app_server.utils.logger.os.environ', environ, clear=True
    ):
...
class TestJsonOutput:
    ...
    def test_extra_fields_from_adapter(self, json_handler):
        logger, string_io = json_handler
        subject = OmniAgentLoggerAdapter(logger, extra={'context_field': '..val..'})
    ...
    def test_extra_fields_from_adapter_can_override(self, json_handler):
        logger, string_io = json_handler
        subject = OmniAgentLoggerAdapter(logger, extra={'override': 'a'})
