...
from omniagent.app_server.errors import SandboxDeleteRetryError, SandboxError
...
from omniagent.app_server.settings.settings_models import SandboxGroupingStrategy
...
class TestSandboxLifecycle:
    ...
    async def test_archive_conversation_workspace_archives_without_stopping(
        ...
        with patch(
            'omniagent.app_server.sandbox.remote_sandbox_service.workspace_archive'
        ) as mock_wa:
...
class TestPollAgentServersSessionScoping:
    ...
    def _patches(
        ...
        return [
            patch('omniagent.app_server.config.get_db_session', side_effect=tracker),
            patch(
                'omniagent.app_server.config.get_app_conversation_info_service',
                side_effect=_async_cm_factory(conv_service),
            ),
            patch(
                'omniagent.app_server.config.get_event_service',
                side_effect=_async_cm_factory(event_service),
            ),
            patch(
                'omniagent.app_server.config.get_event_callback_service',
                side_effect=_async_cm_factory(callback_service),
            ),
            patch(
                'omniagent.app_server.config.get_httpx_client',
                side_effect=_async_cm_factory(httpx_client),
            ),
            patch(
                'omniagent.app_server.sandbox.remote_sandbox_service.ConversationInfo',
                mock_conv_info,
            ),
            patch(
                'omniagent.app_server.sandbox.remote_sandbox_service.EventPage',
                mock_event_page,
            ),
            patch('omniagent.app_server.sandbox.remote_sandbox_service.InjectorState'),
            patch('omniagent.app_server.sandbox.remote_sandbox_service.ADMIN'),
            patch(
                'omniagent.app_server.sandbox.remote_sandbox_service.USER_CONTEXT_ATTR',
                'user_context',
            ),
        ]
...
class TestArchiveConversationWorkspace:
    ...
    async def test_required_failure_returns_false(self, remote_sandbox_service):
        ...
        with (
            patch(
                'omniagent.app_server.sandbox.workspace_archive.archive_enabled',
                return_value=True,
            ),
            patch(
                'omniagent.app_server.sandbox.workspace_archive.archive_workspace',
    ...
    async def test_pinned_path_forwarded_to_archive(self, remote_sandbox_service):
        ...
        with (
            ...
            ok = await remote_sandbox_service.archive_conversation_workspace(
                ...
                workspace_path='/home/omniagent/workspace/conv-1',
            )
        assert ok is True
        _, kwargs = mock_archive.call_args
        assert kwargs['archive_path'] == '/home/omniagent/workspace/conv-1'
    ...
    async def test_resolve_archive_path_uses_pinned(self, remote_sandbox_service):
        ...
        path = await remote_sandbox_service._resolve_archive_path(
            stored, 'conv-1', '/home/omniagent/workspace/conv-1'
        )
        assert path == '/home/omniagent/workspace/conv-1'
        remote_sandbox_service.sandbox_spec_service.get_sandbox_spec.assert_not_called()
...
class TestArchiveWorkspaceHelper:
    ...
    async def test_archive_uploads_archive_and_manifest(self, monkeypatch):
        from omniagent.app_server.sandbox import workspace_archive
...
class TestDeleteSandboxKeyHandling:
    ...
    async def async_engine(self):
        ...
        from omniagent.app_server.utils.sql_utils import Base
...
class TestArchiveEnvToggles:
    """Per OmniAgent/AGENTS.md, env enable-toggles must accept '1' as well as
