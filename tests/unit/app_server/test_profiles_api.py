...
from omniagent.app_server.file_store import get_file_store
...
from omniagent.app_server.settings.llm_profiles import MAX_PROFILES_PER_USER
from omniagent.app_server.settings.settings_models import Settings
from omniagent.app_server.settings.settings_router import _user_profile_locks
...
async def test_save_profile_rejects_client_declared_is_subscription(
    ...
    OmniAgent/software-agent-sdk#3942; see ``StrictLLM._restore_is_subscription``.
    """
    await _seed(settings_store, _base_settings())
...
async def test_activate_does_not_mutate_saved_profile_base_url(
    ...
    Use a custom base_url (not an omniagent/* model) so the LLM SDK does
...
async def test_concurrent_writes_all_persist(tmp_path: Path):
    ...
    from omniagent.app_server.settings.settings_router import (
        SaveProfileRequest,
        save_profile,
    )
