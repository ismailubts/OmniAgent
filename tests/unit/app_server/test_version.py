...
class TestGetVersionFromPyproject:
    ...
    def test_reads_first_candidate_path(self, tmp_path):
        ...
        with patch(
            'omniagent.app_server.version.os.path.abspath', return_value=str(fake_file)
        ):
            assert get_version() == '1.2.3'

    def test_reads_second_candidate_path(self, tmp_path):
        # Only place pyproject.toml under the omniagent/ subdirectory
...
class TestGetVersionFallbacks:
    ...
    def _patch_no_pyproject(self):
        ...
        return patch(
            'omniagent.app_server.version.os.path.abspath',
            return_value='/nonexistent/omniagent/app_server/version.py',
        )
...
class TestModuleLevelVersion:
    ...
    def test_module_version_matches_pattern(self):
        from omniagent.app_server.version import __version__
