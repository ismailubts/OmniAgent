"""Regression tests for ``omniagent.app_server.services.db_session``.
...
exactly the crashloop seen in OmniAgent/deploy#4315.
"""
...
from omniagent.app_server.services.db_session import (
    _yield_db_session,
    depends_db_session,
)
...
def test_depends_db_session_does_not_resolve_config_at_construction():
    ...
    We install a sentinel ``omniagent.app_server.config`` module whose
    ...
    """
    original = sys.modules.get('omniagent.app_server.config')
    sentinel = types.ModuleType('omniagent.app_server.config')
    ...
    sys.modules['omniagent.app_server.config'] = sentinel
    try:
        ...
        if original is None:
            sys.modules.pop('omniagent.app_server.config', None)
        else:
            sys.modules['omniagent.app_server.config'] = original
