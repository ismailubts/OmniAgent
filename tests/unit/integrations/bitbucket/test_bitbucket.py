...
from omniagent.app_server.integrations.service_types import OwnerType, Repository
from omniagent.app_server.integrations.service_types import (
    ProviderType as ServiceProviderType,
)
from omniagent.app_server.integrations.utils import validate_provider_token
from omniagent.app_server.secrets.secrets_router import check_provider_tokens
from omniagent.app_server.settings.settings_models import POSTProviderModel
...
async def test_validate_provider_token_with_bitbucket_token():
    ...
    with (
        patch(
            'omniagent.app_server.integrations.utils.GitHubService'
        ) as mock_github_service,
        patch(
            'omniagent.app_server.integrations.utils.GitLabService'
        ) as mock_gitlab_service,
        patch(
            'omniagent.app_server.integrations.utils.BitBucketService'
...
async def test_check_provider_tokens_with_only_bitbucket():
    ...
    with patch(
        'omniagent.app_server.secrets.secrets_router.validate_provider_token',
        mock_validate,
    ):
...
async def test_resolve_primary_email_selects_primary_confirmed():
    ...
    from omniagent.app_server.integrations.bitbucket.service.base import (
        BitBucketMixinBase,
    )
