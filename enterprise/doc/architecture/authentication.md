...
OmniAgent uses Keycloak for identity management in the SaaS deployment. The authentication flow involves multiple services:
...
| **UserAuth** | Abstract auth interface | `omniagent/server/user_auth/user_auth.py` |
| **SaasUserAuth** | Keycloak implementation | `enterprise/server/auth/saas_user_auth.py` |
| **JWT Service** | Token signing/verification | `omniagent/app_server/services/jwt_service.py` |
