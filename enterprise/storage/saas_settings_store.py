...
from omniagent.app_server.settings.llm_profiles import LLMProfiles, resolve_profile_llm
...
from omniagent.app_server.utils.jsonpatch_compat import (
    deep_merge,
    deep_merge_with_wholesale_keys,
)
from omniagent.app_server.utils.llm import is_openhands_model
from openhands.sdk.llm.utils.omniagent_provider import (
...
class ManagedLlmKeyConfig:
    """Whether an effective LLM config uses a managed LiteLLM/OmniAgent key."""
...
def managed_llm_key_config_from_model(
    ...
    Returns ``None`` when the config is not a managed LiteLLM/OmniAgent-provider
    ...
    uses_openhands_provider_proxy = openhands_type and (
        ...
        or 'github.com/ismailubts/OmniAgent' in normalized_llm_base_url.lower()
    )
...
class SaasSettingsStore(SettingsStore):
    ...
    def _resolve_active_agent_profile(
        ...
        try:
            ...
            # (OmniAgent kind only), so managed OmniAgent keys and provider-default
            ...
            # Canonicalize legacy managed OmniAgent model names/base_urls on the
    ...
    async def load(
        ...
        *,
        ...
        # Canonicalize legacy managed OmniAgent LLM payloads before Settings
    ...
    async def _ensure_api_key(
        ...
        """Generate and set the OmniAgent API key for the given settings.
        ...
        if not existing_key_valid:
            ...
            # the default to/from an omniagent/* model never orphans a key.
    ...
    async def rotate_managed_llm_key(self) -> ManagedLlmKeyRotation:
        """Force-rotate the managed LiteLLM/OmniAgent key for this user/org.
        ...
        ``omniagent/*`` model, matching ``verify_existing_key``'s contract.
        ...
        async with a_session_maker() as session:
            ...
            new_key = await LiteLlmManager.generate_key(
                ...
                org_id_str,
                key_alias,
                {'type': 'openhands'} if config.omniagent_type else None,
            )
            ...
            return ManagedLlmKeyRotation(
                ...
                old_key=old_key,
                new_key=new_key,
                openhands_type=config.omniagent_type,
            )
