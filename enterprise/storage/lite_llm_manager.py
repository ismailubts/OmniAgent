...
def get_openhands_cloud_key_alias(keycloak_user_id: str, org_id: str) -> str:
    """Generate the key alias for OmniAgent Cloud managed keys."""
    return f'OmniAgent Cloud - user {keycloak_user_id} - org {org_id}'
    ...
    async def create_entries(
        ...
        if not local_deploy:
            ...
            async with httpx.AsyncClient(
                ...
                if add_user_to_team:
                    if create_user:
                        # create_user is True only when no OmniAgent User row exists,
    ...
    async def _verify_existing_key(
        ...
        for key_info in keys:
            ...
            if (
                ...
                # Found an existing OmniAgent key for this org
