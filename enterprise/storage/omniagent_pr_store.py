...
class OpenhandsPRStore:
    ...
    async def update_pr_openhands_stats(
        ...
        """
        Update OmniAgent statistics for a PR with row-level locking and timestamp validation.

        Args:
            repo_id: Repository identifier
            pr_number: Pull request number
            original_updated_at: Original updated_at timestamp to check for concurrent modifications
            openhands_helped_author: Whether OmniAgent helped the author (1+ commits)
            num_openhands_commits: Number of commits by OmniAgent
            num_openhands_review_comments: Number of review comments by OmniAgent
            num_openhands_general_comments: Number of PR comments (not review comments) by OmniAgent
        ...
        async with a_session_maker() as session:
            ...
            # Update the OmniAgent statistics
            pr.omniagent_helped_author = openhands_helped_author
