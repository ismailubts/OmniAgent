...
class SlackManager(Manager[SlackViewInterface]):
    def __init__(self, token_manager):
        ...
        self.login_link = (
            'User has not yet authenticated: [Click here to Login to OmniAgent]({}).'
        )

        self.jinja_env = Environment(
            loader=FileSystemLoader(OMNIAGENT_RESOLVER_TEMPLATES_DIR + 'slack')
        )
    ...
    async def authenticate_user(
        ...
        # We get the user and correlate them back to a user in OmniAgent - if we can
        slack_user = None
    ...
    async def start_job(self, slack_view: SlackViewInterface) -> None:
        ...
        try:
            ...
            try:
                ...
                msg_info = f'{user_info.slack_display_name} please re-login into [OmniAgent Cloud]({HOST_URL}) before starting a job.'
                ...
                msg_info = f'@{user_info.slack_display_name} please set a valid LLM API key in [OmniAgent Cloud]({HOST_URL}) before starting a job.'
