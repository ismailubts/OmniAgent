...
from omniagent.app_server.integrations.provider import ProviderHandler
from omniagent.app_server.integrations.service_types import Comment, Repository
from omniagent.app_server.types import (
    LLMAuthenticationError,
    MissingSettingsError,
    SessionExpiredError,
)
from omniagent.app_server.user_auth.user_auth import UserAuth
from omniagent.app_server.utils.http_session import httpx_verify_option
...
# Events the OmniAgent webhook subscribes to, used when auto-enrolling the
...
def _comment_addresses_openhands(comment: str, bot_ids: set[str] | None) -> bool:
    # Literal @omniagent always triggers (works even when the bot identity can't
    ...
    if not comment:
        return False
    # Boundary-aware + case-insensitive: matches @OmniAgent but not an email like
    # someone@users.noreply.github.com (incl. the service account's own address).
...
class JiraDcManager(Manager[JiraDcViewInterface]):
    def __init__(self, token_manager: TokenManager):
        ...
        self.jinja_env = Environment(
            loader=FileSystemLoader(OMNIAGENT_RESOLVER_TEMPLATES_DIR + 'jira_dc')
        )
    ...
    async def authenticate_user(
        ...
        """Authenticate Jira DC user and get their OmniAgent user auth."""
        ...
        # email to their OmniAgent email instead. In OAuth mode we resolve
        ...
        if not JIRA_DC_ENABLE_OAUTH or not jira_dc_user_id or jira_dc_user_id == 'none':
            ...
            # matches an OmniAgent account is auto-enrolled, no manual link step.
    ...
    async def receive_message(self, message: Message):
        ...
        if not jira_dc_user or not saas_user_auth:
            ...
            # Distinguish "no OmniAgent account" from "account exists but not linked
    ...
    async def start_job(self, jira_dc_view: JiraDcViewInterface) -> None:
        ...
        try:
            ...
            msg_info = f'Please re-login into [OmniAgent Cloud]({HOST_URL}) before starting a job.'

        except LLMAuthenticationError as e:
            logger.warning(f'[Jira DC] LLM authentication error: {str(e)}')
            msg_info = f'Please set a valid LLM API key in [OmniAgent Cloud]({HOST_URL}) before starting a job.'
    ...
    async def _resolve_service_account_mentions(self, payload: Dict) -> set[str] | None:
        ...
        mentions pay a lookup; literal @omniagent and non-comment payloads
    ...
    async def get_issue_comments(
        ...
        ``Comment.system`` so the prompt can label them as OmniAgent' own prior
    ...
    async def _add_acknowledgement_reaction(
        ...
        """Acknowledge the @omniagent mention with a best-effort reaction.
    ...
    async def register_webhook(
        ...
        """Create or update the OmniAgent webhook in Jira DC via the admin API.
        ...
        Args:
            ...
            events_url: The OmniAgent endpoint Jira should POST events to.
    ...
    async def delete_webhook(
        ...
        """Delete the OmniAgent webhook from Jira DC, if present.
        ...
        Args:
            ...
            admin_api_key: A Jira admin PAT. Used only for this call; never
                stored.
            events_url: The OmniAgent endpoint whose webhook should be removed.
        ...
        try:
            ...
            if not mentioned_repos:
                ...
                comment_msg = (
                    ...
                    'linked to your OmniAgent user can access it.'
                )
