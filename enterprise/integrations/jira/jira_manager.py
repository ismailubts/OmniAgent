...
class JiraManager(Manager[JiraViewInterface]):
    ...
    def __init__(self, token_manager: TokenManager):
        ...
        self.jinja_env = Environment(
            loader=FileSystemLoader(OMNIAGENT_RESOLVER_TEMPLATES_DIR + 'jira')
        )
    ...
    async def _authenticate_user(
        ...
        """Authenticate the Jira user and get OmniAgent auth."""
        ...
        if not saas_user_auth:
            logger.warning(
                '[Jira] Failed to get OmniAgent auth',
                extra={
            ...
            await self._send_error_from_payload(
                payload,
                workspace,
                f'User {payload.user_email} is not authenticated with OmniAgent.',
            )
            return None, None
