...
from omniagent.app_server.integrations.service_types import Repository
...
def get_session_expired_message(username: str | None = None) -> str:
    ...
    """
    if username:
        return f'@{username} your session has expired. Please login again at [OmniAgent Cloud]({HOST_URL}) and try again.'
    return f'Your session has expired. Please login again at [OmniAgent Cloud]({HOST_URL}) and try again.'


def get_user_not_found_message(username: str | None = None) -> str:
    """Get a user-friendly message when a user hasn't created an OmniAgent account.

    Used by integrations to notify users when they try to use OmniAgent features
    but haven't logged into OmniAgent Cloud yet (no Keycloak account exists).
    ...
    """
    if username:
        return f"@{username} it looks like you haven't created an OmniAgent account yet. Please sign up at [OmniAgent Cloud]({HOST_URL}) and try again."
    return f"It looks like you haven't created an OmniAgent account yet. Please sign up at [OmniAgent Cloud]({HOST_URL}) and try again."


def get_account_not_linked_message(username: str | None = None) -> str:
    """Get a message when a user has an OmniAgent account but hasn't linked it.
    ...
    """
    if username:
        return f"@{username} you have an OmniAgent account but haven't linked it to this workspace yet. Please link it at [OmniAgent Cloud]({HOST_URL}) under Settings > Integrations and try again."
    return f"You have an OmniAgent account but haven't linked it to this workspace yet. Please link it at [OmniAgent Cloud]({HOST_URL}) under Settings > Integrations and try again."
...
def get_jira_dc_relink_message(display_name: str | None = None) -> str:
    ...
    return (
        ...
        f'Please re-link in [OmniAgent Cloud|{HOST_URL}] under '
        f'Settings → Integrations to continue using OmniAgent from Jira.'
    )


OMNIAGENT_RESOLVER_TEMPLATES_DIR = (
    os.getenv('OMNIAGENT_RESOLVER_TEMPLATES_DIR')
    or 'omniagent/app_server/integrations/templates/resolver/'
)
_jinja_env = Environment(loader=FileSystemLoader(OMNIAGENT_RESOLVER_TEMPLATES_DIR))


def get_oh_labels(web_host: str) -> tuple[str, str]:
    """Get the OmniAgent labels based on the web host.
    ...
    inline_oh_label = '@omniagent-exp' if is_staging_or_local else '@omniagent'
...
def has_exact_mention(text: str, mention: str) -> bool:
    ...
    Args:
        ...
        mention: The mention to look for (e.g. "@omniagent")

    Returns:
        bool: True if the exact mention is found, False otherwise

    Example:
        >>> has_exact_mention("Hello @omniagent!", "@omniagent")  # True
        >>> has_exact_mention("Hello @omniagent-agent!", "@omniagent")  # False
        >>> has_exact_mention("(@omniagent)", "@omniagent")  # True
        >>> has_exact_mention("user@omniagent.com", "@omniagent")  # False
        >>> has_exact_mention("Hello @OmniAgent!", "@omniagent")  # True (case-insensitive)
    """
