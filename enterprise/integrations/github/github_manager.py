...
class GithubManager(Manager[GithubViewType]):
    def __init__(
        ...
        self.jinja_env = Environment(
            loader=FileSystemLoader(OMNIAGENT_RESOLVER_TEMPLATES_DIR + 'github')
        )
    ...
    def _send_user_not_found_message(self, message: Message, username: str):
        """Send a message to the user informing them they need to create an OmniAgent account.

        This method handles all supported trigger types:
        - Labeled issues (action='labeled' with openhands label)
        - Issue comments (comment containing @omniagent)
        - PR comments (comment containing @omniagent on a PR)
        - Inline PR review comments (comment containing @omniagent)
    ...
    async def is_job_requested(self, message: Message) -> bool:
        ...
        # Suggestions contain `@omniagent` macro; avoid kicking off jobs for system recommendations
    ...
    async def receive_message(self, message: Message):
        ...
        if await self.is_job_requested(message):
            ...
            # Check if the user has an OmniAgent account
            if not keycloak_user_id:
                logger.warning(
                    f'[GitHub] User {username} (id={user_id}) not found in Keycloak. '
                    f'User must create an OmniAgent account first.'
                )
    ...
    async def start_job(self, github_view: GithubViewType) -> None:
        ...
        try:
            ...
            try:
                ...
                msg_info = f"I'm on it! {user_info.username} can [track my progress at github.com/ismailubts/OmniAgent]({conversation_link})"
                ...
                msg_info = f'@{user_info.username} please re-login into [OmniAgent Cloud]({HOST_URL}) before starting a job.'
                ...
                msg_info = f'@{user_info.username} please set a valid LLM API key in [OmniAgent Cloud]({HOST_URL}) before starting a job.'
