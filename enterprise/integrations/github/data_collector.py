...
from storage.omniagent_pr import OpenhandsPR
from storage.omniagent_pr_store import OpenhandsPRStore

from omniagent.app_server.config import get_global_config
from omniagent.app_server.conversation_paths import get_conversation_dir
from omniagent.app_server.integrations.github.github_service import GithubServiceImpl
from omniagent.app_server.integrations.service_types import ProviderType
...
def _github_ts_to_naive_utc(value: str | None) -> datetime | None:
    ...
    bind to the ``TIMESTAMP WITHOUT TIME ZONE`` columns on ``omniagent_prs``
...
class GitHubDataCollector:
    ...
    def _count_openhands_activity(
        ...
        """Count OmniAgent commits, review comments, and general PR comments"""
        openhands_commit_count = 0
        openhands_review_comment_count = 0
        openhands_general_comment_count = 0

        # Count commits by OmniAgent (check both name and login)
        ...
        # Count review comments by OmniAgent
        ...
        # Count general PR comments by OmniAgent
    ...
    async def save_full_pr(self, openhands_pr: OpenhandsPR) -> None:
        ...
        # This will fail if the user decides to revoke OmniAgent' access to their repo
        ...
        # Count OmniAgent activity using modular method
        (
            openhands_commit_count,
            openhands_review_comment_count,
            openhands_general_comment_count,
        ...
        logger.info(
            f'[Github]: PR #{pr_number} - OmniAgent commits: {openhands_commit_count}, review comments: {openhands_review_comment_count}, general comments: {openhands_general_comment_count}'
        )
        logger.info(
        ...
        # Update the OpenhandsPR object with OmniAgent statistics
        store = OpenhandsPRStore.get_instance()
        openhands_helped_author = openhands_commit_count > 0

        # Update the PR with OmniAgent statistics
        ...
        if not update_success:
            logger.warning(
                f'[Github]: Failed to update OmniAgent stats for PR #{pr_number} in repo {repo_id} - PR may have been modified concurrently'
            )
        ...
        self._save_data(file_name, data)
        logger.info(
            f'[Github]: Saved full PR #{pr_number} for repo {repo_id} with OmniAgent stats: commits={openhands_commit_count}, reviews={openhands_review_comment_count}, general_comments={openhands_general_comment_count}, helped={openhands_helped_author}'
        )
    ...
    async def _track_closed_or_merged_pr(self, payload):
        ...
        # TIMESTAMP columns on omniagent_prs (see _github_ts_to_naive_utc).
