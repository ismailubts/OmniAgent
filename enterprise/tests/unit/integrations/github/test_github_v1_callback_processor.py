...
class TestGithubV1CallbackProcessor:
    ...
    @patch('omniagent.app_server.config.get_app_conversation_info_service')
    @patch('omniagent.app_server.config.get_sandbox_service')
    @patch('omniagent.app_server.config.get_httpx_client')
    ...
    async def test_exception_handling_posts_error_to_github(
        ...
        assert (
            'OmniAgent encountered an error: **Simulated agent server error**'
            in error_comment
        )
    ...
    async def test_budget_exceeded_error_logs_info_and_sends_friendly_message(
        ...
        assert 'OmniAgent encountered an error' in posted_comment
