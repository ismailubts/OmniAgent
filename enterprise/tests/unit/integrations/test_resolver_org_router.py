...
Tests the resolve_org_for_repo function which determines which OmniAgent
...
async def test_returns_org_id_when_claimed_and_user_is_member(mock_stores):
    ...
    # Act
    result = await resolve_org_for_repo('github', 'OmniAgent/foo', USER_ID)
...
async def test_returns_org_id_without_membership_check_when_no_user_id(mock_stores):
    ...
    result = await resolve_org_for_repo('github', 'OmniAgent/foo')
