...
def _comment_message(*, body: str = 'Hey @omniagent fix') -> Message:
    return Message(
...
def _pr_comment_view(parent_id: int | None = 42) -> BitbucketDCPRComment:
    return BitbucketDCPRComment(
        ...
        comment_id=99,
        comment_body='Hey @omniagent fix',
...
@pytest.mark.asyncio
@pytest.mark.parametrize('actor_slug', ['openhands', 'OmniAgent'])
async def test_receive_message_skips_event_authored_by_bot(actor_slug):
    ...
    The agent's reply is posted via the bot PAT and can contain "@omniagent";
...
def test_posting_service_raises_when_bot_token_unset():
    ...
    reply with "@omniagent" would re-fire the webhook), so building the posting
