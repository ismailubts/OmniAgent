...
def jinja_env() -> Environment:
    ...
    return Environment(
        loader=FileSystemLoader(
            str(
                repo_root
                / 'omniagent/app_server/integrations/templates/resolver/bitbucket'
            )
        )
    )
...
async def test_factory_creates_pr_comment_view_for_pr_comment_added_with_mention():
    msg = _make_message(body='Hey @omniagent please fix typo', parent_id=42)
...
def test_is_pr_comment_returns_false_for_unknown_event_key():
    msg = _make_message(body='@omniagent fix', event_key='repo:refs_changed')
    assert BitbucketDCFactory.is_pr_comment(msg) is False


def test_is_pr_comment_returns_false_for_edited_event():
    # Created-only: editing an @omniagent comment must not re-trigger a job.
    msg = _make_message(body='@omniagent fix', event_key='pr:comment:edited')
    assert BitbucketDCFactory.is_pr_comment(msg) is False


def test_is_pr_comment_returns_true_for_added_event_with_mention():
    msg = _make_message(body='@omniagent fix', event_key='pr:comment:added')
...
async def test_factory_records_actor_slug_and_assigns_keycloak_user_id():
    msg = _make_message(body='@omniagent fix')
...
async def test_pr_comment_instructions_include_context_and_actionable_comment(
    ...
    msg = _make_message(body='@omniagent please update the tests')
    view = await BitbucketDCFactory.create_bitbucket_dc_view_from_payload(
    ...
    assert '@omniagent please update the tests' in user_instructions
