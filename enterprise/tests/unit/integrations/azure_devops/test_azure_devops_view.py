...
def jinja_env() -> Environment:
    ...
    return Environment(
        loader=FileSystemLoader(
            str(
                repo_root
                / 'omniagent/app_server/integrations/templates/resolver/azure_devops'
            )
        )
    )


def _make_pr_message(body: str = '@omniagent please fix this') -> Message:
    return Message(
...
def _make_work_item_message(body: str = '@omniagent please fix work item') -> Message:
    return Message(
        source=SourceType.AZURE_DEVOPS,
        message={
...
def test_is_pr_comment_requires_exact_mention():
    ...
    assert (
        AzureDevOpsFactory.is_pr_comment(
            _make_pr_message(mark_openhands_comment('@omniagent generated summary'))
        )
        is False
    )
    ...
    assert (
        AzureDevOpsFactory.is_work_item_comment(
            _make_work_item_message(
                mark_openhands_comment('@omniagent generated summary')
            )
        )
        is False
    )
...
async def test_factory_work_item_view_derives_real_repo(monkeypatch):
    ...
    assert view.comment_body == '@omniagent please fix work item'
...
async def test_pr_comment_instructions_include_actionable_comment(jinja_env):
    ...
    assert '@omniagent please fix this' in user_instructions
...
async def test_pr_comment_load_resolver_context_uses_pr_not_work_item(monkeypatch):
    # A brand-new PR whose only comment is the @omniagent mention returns an empty
