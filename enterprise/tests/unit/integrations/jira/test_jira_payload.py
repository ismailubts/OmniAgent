...
@pytest.fixture
def parser():
    """Create a JiraPayloadParser with standard OmniAgent labels."""
    return JiraPayloadParser(oh_label='openhands', inline_oh_label='@omniagent')


@pytest.fixture
def valid_label_payload():
    """Create a valid jira:issue_updated payload with OmniAgent label."""
    return {
...
def valid_comment_payload():
    """Create a valid comment_created payload with OmniAgent mention."""
    return {
        ...
        'comment': {
            'body': '@omniagent please fix this bug',
            'author': {
                'displayName': 'Test User',
                'accountId': 'account-123',
                'emailAddress': 'test@example.com',
...
class TestLabelFiltering:
    """Tests for OmniAgent label filtering."""

    def test_label_event_without_openhands_label_skipped(
        self, parser, valid_label_payload
    ):
        """Verify label events without OmniAgent label are skipped."""
...
class TestCommentFiltering:
    """Tests for OmniAgent comment mention filtering."""

    def test_comment_without_mention_skipped(self, parser, valid_comment_payload):
        """Verify comments without OmniAgent mention are skipped."""
        ...
        # Assert
        assert isinstance(result, JiraPayloadSkipped)
        assert '@omniagent' in result.skip_reason
