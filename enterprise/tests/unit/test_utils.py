...
def test_has_exact_mention():
    ...
    assert has_exact_mention('Hello @omniagent!', '@omniagent') is True
    assert has_exact_mention('@omniagent at start', '@omniagent') is True
    assert has_exact_mention('End with @omniagent', '@omniagent') is True
    assert has_exact_mention('@omniagent', '@omniagent') is True

    # Test no match
    assert has_exact_mention('No mention here', '@omniagent') is False
    assert has_exact_mention('', '@omniagent') is False

    # Test partial matches (should be False)
    assert has_exact_mention('Hello @omniagent-agent!', '@omniagent') is False
    assert has_exact_mention('Email: user@omniagent.com', '@omniagent') is False
    assert has_exact_mention('Text@omniagent', '@omniagent') is False
    assert has_exact_mention('@omniagentmore', '@omniagent') is False
    ...
    assert has_exact_mention('Hi @OmniAgent', '@OmniAgent') is True
    assert has_exact_mention('Hi @OmniAgent', '@omniagent') is True
    assert has_exact_mention('Hi @omniagent', '@OmniAgent') is True
    assert has_exact_mention('Hi @OPENHANDS', '@omniagent') is True

    # Test multiple mentions
    assert has_exact_mention('@omniagent and @omniagent again', '@omniagent') is True
    assert has_exact_mention('@omniagent-agent and @omniagent', '@omniagent') is True

    # Test with surrounding punctuation
    assert has_exact_mention('Hey, @omniagent!', '@omniagent') is True
    assert has_exact_mention('(@omniagent)', '@omniagent') is True
    assert has_exact_mention('@omniagent: hello', '@omniagent') is True
    assert has_exact_mention('@omniagent? yes', '@omniagent') is True
...
def test_infer_repo_from_message():
    test_cases = [
        ...
        (
            'Check out https://github.com/ismailubts/OmniAgent.git for details',
            ['ismailubts/OmniAgent'],
        ),
        ...
        ('Please deploy the ismailubts/OmniAgent repo', ['ismailubts/OmniAgent']),
        ...
        (
            '@omniagent-exp just echo hello world in `ismailubts/OmniAgent-CLI` repository',
            ['ismailubts/OmniAgent-CLI'],
        ),
        (
            '@omniagent-exp echo hello world with {{ismailubts/OmniAgent-CLI}}',
            ['ismailubts/OmniAgent-CLI'],
        ),
