...
from omniagent.app_server.utils.llm import ModelsResponse
...
# guard; an ``omniagent/``-prefixed entry must be dropped from the union.
...
class TestHappyPath:
    async def test_models_response_shape(self, monkeypatch):
        ...
        # omniagent/ prefix.
        assert response.models == ['openhands/claude-sonnet-4-5', 'omniagent/gpt-5']
        ...
        assert response.hidden_models == ['omniagent/hidden-model']
        # The alias carries its canonical mapping (canonical is visible).
        assert response.hidden_model_canonicals == {
            'omniagent/hidden-model': 'openhands/claude-sonnet-4-5'
        }
        # litellm_proxy/ prefix translated to omniagent/.
    ...
    async def test_hidden_only_when_all_duplicate_entries_hidden(self, monkeypatch):
        ...
        assert response.models == ['omniagent/mixed-model']
        assert response.hidden_models == ['omniagent/legacy-alias']
    ...
    async def test_canonical_skipped_when_not_visible(self, monkeypatch):
        ...
        assert response.models == ['omniagent/real-model']
        assert response.hidden_models == [
            'omniagent/alias-to-missing',
            'omniagent/alias-to-hidden',
    ...
    async def test_canonical_mapped_for_visible_target(self, monkeypatch):
        ...
        assert response.hidden_model_canonicals == {
            'omniagent/legacy-alias': 'omniagent/real-model'
        }
...
class TestDefaultModelTranslation:
    def test_litellm_proxy_prefix_translated(self, monkeypatch):
        ...
        assert _derive_default_model() == 'omniagent/gpt-5'

    def test_openhands_prefix_preserved(self, monkeypatch):
        monkeypatch.setattr(
            router_module, 'get_default_litellm_model', lambda: 'omniagent/gpt-5'
        )
        assert _derive_default_model() == 'omniagent/gpt-5'
...
class TestFailurePaths:
    ...
    async def test_stale_cache_served_on_failure(self, monkeypatch, fake_clock, caplog):
        ...
        assert first.models == ['openhands/claude-sonnet-4-5', 'omniagent/gpt-5']
...
class TestCacheTtl:
    ...
    async def test_after_ttl_refetches(self, monkeypatch, fake_clock):
        ...
        assert len(calls) == 1
        assert len(calls2) == 1
        assert response.models == ['omniagent/new-model']
...
class TestByokUnion:
    ...
    async def test_byok_on_unions_catalogue(self, monkeypatch):
        ...
        # the omniagent/-prefixed catalogue entry is dropped.
        assert response.models == [
