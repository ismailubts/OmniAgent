...
These tests are designed to work without the full OmniAgent dependency chain.
They test the core logic and behavior of the processor using comprehensive mocking.

To run these tests in an environment with OmniAgent dependencies:
1. Ensure OmniAgent is available in the Python path
...
class TestUserVersionUpgradeProcessorStandalone:
    """Standalone tests for UserVersionUpgradeProcessor without OmniAgent dependencies."""
...
class TestUserVersionUpgradeProcessorIntegration:
    """
    Integration test scenarios for when OmniAgent dependencies are available.

    These tests would require:
    1. OmniAgent to be installed and available
