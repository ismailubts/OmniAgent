...
from omniagent.app_server.user_auth.user_auth import AuthType
...
def mock_config():
    ...
    from omniagent.app_server.utils.encryption_key import EncryptionKey
...
# =============================================================================
# Tests for OMNIAGENT_API_KEY injection
# =============================================================================


class TestOmniAgentApiKey:
    """Tests for OMNIAGENT_API_KEY system secret generation and injection."""
    ...
    async def test_get_secrets_includes_openhands_api_key(self):
        """Test that get_secrets injects OMNIAGENT_API_KEY into custom_secrets."""
        user_id = 'test_user_id'
        org_id = uuid.uuid4()
        expected_api_key = 'sk-oh-test-key-12345'
        ...
        # Create mock secrets from store (without OMNIAGENT_API_KEY)
        ...
        with (
            ...
            assert 'OMNIAGENT_API_KEY' in result.custom_secrets
            assert (
                result.custom_secrets['OMNIAGENT_API_KEY'].secret.get_secret_value()
                == expected_api_key
            )
            assert (
                'system-managed'
                in result.custom_secrets['OMNIAGENT_API_KEY'].description
            )
    ...
    async def test_get_secrets_handles_empty_stored_secrets(self):
        ...
        with (
            ...
            # Assert - should have only OMNIAGENT_API_KEY
            assert result is not None
            assert 'OMNIAGENT_API_KEY' in result.custom_secrets
