...
def test_normalize_agent_settings_masks_api_key_in_json_on_empty_and_real_keys():
    ...
    empty_key = OrgUpdate.model_validate(
        {
            'agent_settings_diff': {
                'llm': {'model': 'omniagent/x', 'api_key': '', 'base_url': None},
            },
        }
    )
...
def test_from_org_validates_persisted_openhands_agent_kind():
    """GIVEN: An org row whose persisted ``agent_settings`` carry the
        canonical ``agent_kind: 'omniagent'`` discriminator (the exact shape
    ...
    org.agent_settings = {
        ...
        'llm': {'model': 'omniagent/claude', 'base_url': LITE_LLM_API_URL},
    }
    ...
    assert response.agent_settings.llm.model == 'omniagent/claude'
...
def test_from_org_keeps_openhands_prefix_and_hides_managed_base_url():
    """Managed OmniAgent models should return the public prefix in basic mode."""
    org = MagicMock(spec=Org)
    org.agent_settings = {
        'schema_version': 1,
        'agent': 'CodeActAgent',
        'llm': {
            'model': 'omniagent/minimax-m2.5',
    ...
    assert response.agent_settings.llm.model == 'omniagent/minimax-m2.5'
...
def test_from_org_returns_provider_default_base_url_as_stored_for_non_managed_models():
    ...
    from omniagent.app_server.utils.llm import get_provider_api_base as _provider_base
