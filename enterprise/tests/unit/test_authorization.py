...
class TestRequireFinancialDataAccess:
    ...
    async def test_grants_access_for_openhands_email(self):
        """
        GIVEN: User with @users.noreply.github.com email
        ...
        user_id = str(uuid4())
        org_id = uuid4()
        mock_request = _create_mock_request_with_email(user_email='admin@users.noreply.github.com')
    ...
    async def test_grants_access_for_owner_role(self):
        """
        GIVEN: User with owner role in organization (non-@users.noreply.github.com email)
        WHEN: require_financial_data_access is called
        THEN: Returns user_id (access granted)
        """
        from server.auth.authorization import require_financial_data_access
    ...
    async def test_grants_access_for_admin_role(self):
        """
        GIVEN: User with admin role in organization (non-@users.noreply.github.com email)
        WHEN: require_financial_data_access is called
        THEN: Returns user_id (access granted)
        """
        from server.auth.authorization import require_financial_data_access
    ...
    async def test_denies_access_for_member_role_without_openhands_email(self):
        """
        GIVEN: User with member role (not admin/owner) and non-@users.noreply.github.com email
        WHEN: require_financial_data_access is called
        ...
        with (
            ...
            assert exc_info.value.status_code == 403
            assert 'admins, owners, or OmniAgent' in exc_info.value.detail
    ...
    async def test_denies_access_when_api_key_org_mismatch(self):
        ...
        mock_request = _create_mock_request_with_email(
            api_key_org_id=api_key_org_id, user_email='admin@users.noreply.github.com'
        )
