...
from omniagent.app_server.settings.settings_models import Settings as DataSettings
...
async def test_ensure_api_key_keeps_valid_key():
    ...
    item = _make_settings(model='omniagent/gpt-4', api_key=existing_key)
...
async def test_ensure_api_key_generates_new_key_when_verification_fails():
    ...
    switching to/from an omniagent/* model never orphans a key."""
    from storage.lite_llm_manager import get_openhands_cloud_key_alias

    store = SaasSettingsStore('test-user-id-123')
    new_key = 'sk-new-key'
    item = _make_settings(model='omniagent/gpt-4', api_key='sk-invalid-key')
...
async def test_store_keeps_openhands_managed_keys_member_specific(
    ...
    """Managed OmniAgent keys should not be copied from one member to everyone else."""
    from sqlalchemy import select
    from storage.org import Org
    from storage.org_member import OrgMember
    ...
    with session_maker() as session:
        org = session.execute(select(Org).where(Org.id == org_id)).scalars().first()
        assert org is not None
        # Settings keeps the public omniagent/ provider prefix in persisted data
        assert (
...
async def test_store_skips_ensure_api_key_for_non_openhands_model_without_base_url(
    ...
    """When saving a non-OmniAgent model with no base URL (basic view BYOR),
...
async def test_store_calls_ensure_api_key_for_openhands_model_without_base_url(
    ...
    """OmniAgent models still require proxy-key verification without a base URL."""
    fixture = org_with_multiple_members_fixture
    admin_user_id = str(fixture['admin_user_id'])
    store = SaasSettingsStore(admin_user_id)
