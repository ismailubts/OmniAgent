...
This service syncs users from the OmniAgent database to a Resend.com audience. It runs as a Kubernetes CronJob that periodically queries the OmniAgent database and adds any new users to the specified Resend audience.

## Features

- Syncs OmniAgent users with email addresses to a Resend.com audience
...
**Failure modes:** If the OmniAgent database is unavailable, the sync job exits with status code 1 and logs the failure. No contacts are synced until the database connection is restored. Monitor the CronJob logs for database connection errors.
