#!/usr/bin/env python3
"""Sync OmniAgent users to a Resend.com audience.

This script reads users from the OmniAgent application database and adds them to
...
- RESEND_FROM_EMAIL: Email address to use as the sender
  (default: "OmniAgent Team <no-reply@welcome.omniagent.dev>")
- RESEND_REPLY_TO_EMAIL: Email address for replies (default: "contact@users.noreply.github.com")
...
from omniagent.app_server.utils.logger import omniagent_logger as logger
...
def _get_session_maker():
    ...
    from omniagent.app_server.config import get_global_config
...
def get_local_users(offset: int = 0, limit: int = BATCH_SIZE) -> list[ResendUser]:
    """Get users with email addresses from the OmniAgent database."""
...
def get_total_local_users() -> int:
    """Get the total number of OmniAgent users with email addresses."""
    session_maker = _get_session_maker()
    with session_maker() as session:
        return (
            session.execute(
...
def send_welcome_email(
    ...
    try:
        ...
        params = {
            'from': os.environ.get(
                'RESEND_FROM_EMAIL', 'OmniAgent Team <no-reply@welcome.omniagent.dev>'
            ),
            'reply_to': os.environ.get(
                'RESEND_REPLY_TO_EMAIL', 'contact@users.noreply.github.com'
            ),
            'to': [email],
            'subject': 'Welcome to OmniAgent Cloud',
            'html': f"""
            <div>
                <p>{greeting}</p>
                <p>Thanks for joining OmniAgent Cloud — we're excited to help you start building with the world's leading open source AI coding agent!</p>
                <p><strong>Here are three quick ways to get started:</strong></p>
                <ol>
                    <li><a href="https://github.com/ismailubts/OmniAgent/blob/main/usage/cloud/openhands-cloud#next-steps"><strong>Connect your Git repo</strong></a> – Link your <a href="https://github.com/ismailubts/OmniAgent/blob/main/usage/cloud/github-installation">GitHub</a> or <a href="https://github.com/ismailubts/OmniAgent/blob/main/usage/cloud/gitlab-installation">GitLab</a> repository in seconds so OmniAgent can begin understanding your codebase and suggest tasks.</li>
                    <li><a href="https://github.com/ismailubts/OmniAgent/blob/main/usage/cloud/github-installation#working-on-github-issues-and-pull-requests-using-openhands"><strong>Use OmniAgent on an issue or pull request</strong></a> – Label an issue with 'openhands' or mention @omniagent on any PR comment to generate explanations, tests, refactors, or doc fixes tailored to the exact lines you're reviewing.</li>
                ...
                <p>Happy coding!</p>
                <p>The OmniAgent team</p>
                <p>--</p>
                <p>OmniAgent</p>
                <p>24 Oak Street</p>
                <p>Cambridge MA 02139</p>
                <p>https://github.com/ismailubts/OmniAgent</p>
            </div>
            """,
        }
...
def sync_users_to_resend():
    """Sync OmniAgent users to Resend.

    This function syncs users from the OmniAgent database to a Resend audience.
    ...
    logger.info(
        f'Starting sync of OmniAgent users to Resend audience {RESEND_AUDIENCE_ID}'
    )
    ...
    try:
        ...
        logger.info(f'Found {total_users} users with emails in OmniAgent database')
