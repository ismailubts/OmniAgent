...
from omniagent.app_server.user_auth import get_user_auth, get_user_id
...
async def get_admin_user_id(
    ...
    """
    Dependency that validates user has @users.noreply.github.com email domain.
    ...
    by checking for @users.noreply.github.com email domain.
    ...
    Raises:
        HTTPException: 403 if email domain is not @users.noreply.github.com
    ...
    if not user_email.endswith('@users.noreply.github.com'):
        logger.warning(
        ...
        raise HTTPException(
            ...
            detail='Access restricted to @users.noreply.github.com users',
        )
...
async def get_org_creator_user_id(
    ...
    - When **disabled** (the default), this falls back to ``get_admin_user_id``
      so org creation remains restricted to ``@users.noreply.github.com`` admin users.
    ...
    Raises:
        ...
        HTTPException: 403 if the feature switch is disabled and the user is
            not an ``@users.noreply.github.com`` admin
