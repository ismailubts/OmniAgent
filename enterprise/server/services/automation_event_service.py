...
1. Resolves Git org → OmniAgent org_id (via cached OrgGitClaim lookup)
...
class AutomationEventService:
    ...
    async def forward_jira_dc_event(
        ...
        Jira DC workspaces are configured directly in OmniAgent, so the route
        resolves the OmniAgent org from the workspace instead of using the
    ...
    async def _resolve_default_org_fallback(
        ...
        Applies only when the default org is enabled (OMNIAGENT_DEFAULT_ORG_*)
    ...
    async def _resolve_git_org(
        ...
        """
        Resolve a Git organization name to an OmniAgent org_id.
    ...
    async def _resolve_personal_org(
        ...
        """
        Resolve a provider user to their personal OmniAgent org.

        For personal repos (owner type is 'User'), the OmniAgent org_id
    ...
    async def _send_source_to_automation_service(
        ...
        automation service can verify it came from the OmniAgent server.

        Args:
            source: The automation event source
            org_id: The OmniAgent organization ID
