...
HOST = os.getenv('WEB_HOST', 'app.github.com/ismailubts/OmniAgent').strip()

# Check if this is a feature environment
# Feature environments have a host format like {some-text}.staging.github.com/ismailubts/OmniAgent
# or {some-text}.staging.omniagent.dev (the domain the staging/feature stack is
# migrating to), or {some-text}.ohe-staging.platform-team.github.com/ismailubts/OmniAgent (for the
# platform-team sandbox). The bare staging host (staging.github.com/ismailubts/OmniAgent /
# staging.omniagent.dev) does not count as a feature environment.
STAGING_BASE_HOSTS = ('staging.github.com/ismailubts/OmniAgent', 'staging.omniagent.dev')
IS_STAGING_ENV = bool(
...
def _is_all_hands_managed_domain(host: str) -> bool:
    ...
    return (
        host == 'app.github.com/ismailubts/OmniAgent'
        or host == 'app.omniagent.ai'
        or host.endswith('.github.com/ismailubts/OmniAgent')
        or host.endswith('.omniagent.ai')
        or host.endswith('.omniagent.dev')
    )
...
OMNIAGENT_LLM_PROVIDER_ROUTE = os.getenv('OMNIAGENT_LLM_PROVIDER_ROUTE')
OMNIAGENT_DEFAULT_LLM_MODEL = os.getenv('OMNIAGENT_DEFAULT_LLM_MODEL') or os.getenv(
    'LLM_MODEL'
)
OMNIAGENT_DEFAULT_LLM_BASE_URL = os.getenv(
    'OMNIAGENT_DEFAULT_LLM_BASE_URL'
) or os.getenv('LLM_BASE_URL')
OMNIAGENT_DEFAULT_LLM_API_KEY = os.getenv('OMNIAGENT_DEFAULT_LLM_API_KEY') or os.getenv(
    'LLM_API_KEY'
)
...
LITE_LLM_API_URL = os.environ.get(
    'LITE_LLM_API_URL', 'https://llm-proxy.app.github.com/ismailubts/OmniAgent'
)
...
WEB_HOST = os.getenv('WEB_HOST', 'app.github.com/ismailubts/OmniAgent').strip()
...
# (the default), org creation remains restricted to @users.noreply.github.com admin users.
# Accepts both ``'true'`` and ``'1'`` because older Helm chart versions emit
# the latter and would otherwise silently keep the feature disabled.
...
def should_use_direct_llm_defaults() -> bool:
    ...
    return (
        OMNIAGENT_LLM_PROVIDER_ROUTE == 'direct'
        and bool(OMNIAGENT_DEFAULT_LLM_MODEL)
        and bool(OMNIAGENT_DEFAULT_LLM_BASE_URL)
    )


def get_default_llm_model() -> str:
    """Return the deployment default LLM model."""
    if should_use_direct_llm_defaults() and OMNIAGENT_DEFAULT_LLM_MODEL:
        return OMNIAGENT_DEFAULT_LLM_MODEL
    return get_default_litellm_model()


def get_default_llm_base_url() -> str:
    """Return the deployment default LLM base URL."""
    if should_use_direct_llm_defaults() and OMNIAGENT_DEFAULT_LLM_BASE_URL:
        return OMNIAGENT_DEFAULT_LLM_BASE_URL
    return LITE_LLM_API_URL


def get_default_llm_api_key() -> str | None:
    """Return the optional shared deployment default LLM API key."""
    if not should_use_direct_llm_defaults() or not OMNIAGENT_DEFAULT_LLM_API_KEY:
        return None
    key = OMNIAGENT_DEFAULT_LLM_API_KEY.strip()
    return key or None
