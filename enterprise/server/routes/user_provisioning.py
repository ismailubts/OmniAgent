...
4. Create the OmniAgent user record (mirroring the ``keycloak_callback``
...
(user-first) would leak the *entire* OmniAgent cascade instead of a
...
OmniAgent ``OrgMember`` row does not propagate to LiteLLM, so on
...
async def provision_user(
    ...
    # have not mutated any OmniAgent state yet, so a clean error is
    # safe to return.
    try:
    ...
    # Anything after this point may need to roll back partial OmniAgent
    # state (DB rows, LiteLLM team, OrgMember rows) **and** the Keycloak
    # user, to avoid leaving an OmniAgent ``User``/personal-``Org``
    ...
    try:
        ...
        # 3. Create the OmniAgent user. Mirrors ``keycloak_callback``'s
...
async def _rollback_partial_provision(
    *,
    ...
    3. **Keycloak user.** Last, so the local OmniAgent identity is
    ...
    # 3. Keycloak user. Always last; runs even when no OmniAgent DB
