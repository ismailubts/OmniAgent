...
from omniagent.app_server.utils.llm import MASKED_API_KEY, resolve_llm_base_url
...
class OrgUpdate(BaseModel):
    ...
    def get_member_updates(self) -> 'OrgMemberSettingsUpdate | None':
        ...
        (e.g. owner switching to a managed/OmniAgent provider). It must not
...
class OrgDefaultsSettingsResponse(BaseModel):
    ...
    def from_org(cls, org: Org) -> 'OrgDefaultsSettingsResponse':
        ...
        The SDK now keeps ``omniagent/`` as the public/stored provider prefix
    ...
    def _prepare_llm_for_response(agent_settings: AgentSettingsConfig) -> None:
        ...
        if (
            llm.model
            and llm.model.startswith('omniagent/')
            and llm.base_url
...
class GitOrgAlreadyClaimedError(Exception):
    """Raised when a Git organization is already claimed by another OmniAgent org."""
