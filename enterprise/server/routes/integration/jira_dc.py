...
from omniagent.app_server.user_auth.user_auth import get_user_auth
...
async def _maybe_register_webhook(
    ...
    """Best-effort auto-enrollment of the OmniAgent webhook in Jira.
...
async def _maybe_delete_webhook(
    ...
    """Best-effort removal of the OmniAgent webhook from Jira during teardown.
...
def _resolve_submitted_service_account(
    ...
    if not svc_acc_email:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail='A service account email is required when configuring Jira DC in OmniAgent.',
        )
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                f'Base URL uses the correct https scheme and that OmniAgent can '
