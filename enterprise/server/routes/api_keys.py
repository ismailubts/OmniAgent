...
async def refresh_managed_llm_api_key(
    ...
    """Refresh the managed OmniAgent LiteLLM key for the current user/org.

    Delegates the full managed-key lifecycle (effective-config classification,
    alias cleanup, key generation with OmniAgent metadata, and persistence) to
    ``SaasSettingsStore.rotate_managed_llm_key`` so the route does not
    duplicate the storage-layer machinery. Only managed LiteLLM/OmniAgent-
