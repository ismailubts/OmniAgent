...
async def get_org_members_financial(
    ...
    - Organization Admins
    - Organization Owners
    - OmniAgent members (users with @users.noreply.github.com emails)
    ...
    Raises:
        HTTPException: 401 if user is not authenticated
        HTTPException: 403 if user lacks access (not admin/owner and not @users.noreply.github.com)
...
async def switch_org(
    ...
    try:
        ...
        if analytics:
            try:
                from omniagent.analytics import resolve_analytics_context
...
async def get_git_claims(
    ...
    """Get all Git organization claims for an OmniAgent organization.

    Only admin and owner roles can view Git organization claims.

    Args:
        org_id: OmniAgent organization UUID
...
async def claim_git_organization(
    ...
    """Claim a Git organization for an OmniAgent organization.

    Only admin and owner roles can claim Git organizations.
    A Git organization can only be claimed by one OmniAgent organization at a time.
...
async def disconnect_git_organization(
    ...
    """Remove a Git organization claim from an OmniAgent organization.
