...
from omniagent.app_server.utils.llm import ModelsResponse, get_supported_llm_models
...
# ``omniagent/`` -> ``litellm_proxy/`` at the transport boundary.
_OMNIAGENT_PREFIX = 'omniagent/'
...
def _derive_default_model() -> str:
    ...
    the app stores and displays models as ``omniagent/<name>``. An already-
    ``omniagent/``-prefixed default is kept as-is; a bare name is prefixed.
    """
    default = get_default_litellm_model()
    if default.startswith(_LITELLM_PROXY_PREFIX):
        return _OMNIAGENT_PREFIX + default[len(_LITELLM_PROXY_PREFIX) :]
    if default.startswith(_OMNIAGENT_PREFIX):
        return default
    return _OMNIAGENT_PREFIX + default
...
class LiteLLMProxyModelService(DefaultLLMModelService):
    ...
    async def _fetch_proxy_model_names(
        ...
        Entries flagged ``model_info.omniagent_hidden`` (legacy alias routes
        ...
        ``model_info.omniagent_canonical`` naming the visible route it
    ...
    def _build_response(
        ...
        return ModelsResponse(
            models=[_OMNIAGENT_PREFIX + name for name in model_names],
            verified_models=list(model_names),
            verified_providers=['openhands'],
            default_model=_derive_default_model(),
            hidden_models=[
                _OMNIAGENT_PREFIX + name for name in hidden_model_names or []
            ],
            hidden_model_canonicals={
                _OMNIAGENT_PREFIX + alias: _OMNIAGENT_PREFIX + canonical
    ...
    async def _union_with_catalogue(
        ...
        extra = [
            ...
            if not m.startswith(_OMNIAGENT_PREFIX) and m not in proxy_models
        ]
        return ModelsResponse(
    ...
    def _is_model_verified(
        ...
        # is not. Require the omniagent/ prefix so a catalogue model whose bare
        # name collides with a proxy one (openai/gpt-5 vs proxy gpt-5) isn't
        # falsely verified.
        return model_name.startswith(_OMNIAGENT_PREFIX) and name in set(
