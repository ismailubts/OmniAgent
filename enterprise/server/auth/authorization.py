...
class Permission(str, Enum):
    ...
    # User provisioning (create new Keycloak/OmniAgent users directly in an org)
...
async def require_financial_data_access(
    ...
    2. User has @users.noreply.github.com email domain
    ...
    # Check if user has @users.noreply.github.com email
    user_auth = await get_user_auth(request)
    user_email = await user_auth.get_user_email()

    if user_email and user_email.endswith('@users.noreply.github.com'):
        logger.debug(
            'Financial data access granted via @users.noreply.github.com email',
    ...
    if user_role.name not in (RoleName.OWNER.value, RoleName.ADMIN.value):
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail='Access restricted to organization admins, owners, or OmniAgent members',
        )
