...
from omniagent.app_server.user_auth.user_auth import AuthType, UserAuth
...
class SaasUserAuth(UserAuth):
    ...
    async def get_secrets(self):
        ...
        # Inject OMNIAGENT_API_KEY (system-level, lazily generated)
        ...
        if openhands_api_key:
            ...
            custom_secrets['OMNIAGENT_API_KEY'] = CustomSecret(
                secret=SecretStr(openhands_api_key),
                description='OmniAgent Cloud API Key for automations and integrations (system-managed)',
            )
    ...
    async def _get_openhands_api_key(self) -> str:
        """Get or create the user's OMNIAGENT_API_KEY (system-level, non-deletable).
