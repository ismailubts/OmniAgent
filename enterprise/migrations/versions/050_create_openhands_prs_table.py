...
def upgrade() -> None:
    ...
    op.create_index(
        'ix_omniagent_prs_repo_id', 'omniagent_prs', ['repo_id'], unique=False
    )
    op.create_index(
        'ix_omniagent_prs_pr_number', 'omniagent_prs', ['pr_number'], unique=False
    )
    op.create_index(
        'ix_omniagent_prs_status', 'omniagent_prs', ['status'], unique=False
    )

    # Create unique constraint on repo_id + pr_number combination
    op.create_index(
        'ix_omniagent_prs_repo_pr_unique',
        'omniagent_prs',
        ['repo_id', 'pr_number'],
        unique=True,
    )
...
def downgrade() -> None:
    op.drop_index('ix_omniagent_prs_repo_id', table_name='omniagent_prs')
    op.drop_index('ix_omniagent_prs_pr_number', table_name='omniagent_prs')
    op.drop_index('ix_omniagent_prs_status', table_name='omniagent_prs')
    op.drop_index('ix_omniagent_prs_repo_pr_unique', table_name='omniagent_prs')
    op.drop_table('omniagent_prs')
