"""Configuration for the OmniAgent App Server."""
...
import omniagent.app_server.event_callback  # noqa: F401
...
from omniagent.app_server.app_lifespan.app_lifespan_service import AppLifespanService
from omniagent.app_server.app_lifespan.oss_app_lifespan_service import (
    OssAppLifespanService,
)
from omniagent.app_server.config_api.config_models import AppMode
from omniagent.app_server.config_api.llm_model_service import (
    LLMModelService,
    LLMModelServiceInjector,
)
from omniagent.app_server.event.event_service import EventService, EventServiceInjector
from omniagent.app_server.event_callback.event_callback_service import (
    EventCallbackService,
    EventCallbackServiceInjector,
)
from omniagent.app_server.file_store.files import FileStore
from omniagent.app_server.file_store.local import LocalFileStore
from omniagent.app_server.pending_messages.pending_message_service import (
    PendingMessageService,
    PendingMessageServiceInjector,
)
from omniagent.app_server.sandbox.sandbox_service import (
    SandboxService,
...
from omniagent.app_server.services.db_session import (  # noqa: F401  (re-exported)
    depends_db_session,
    get_db_session,
)
from omniagent.app_server.services.db_session_injector import (
    DbSessionInjector,
)
from omniagent.app_server.services.httpx_client_injector import HttpxClientInjector
from omniagent.app_server.services.injector import InjectorState
from omniagent.app_server.services.jwt_service import JwtService, JwtServiceInjector
from omniagent.app_server.user.user_context import UserContext, UserContextInjector
from omniagent.app_server.utils.environment import StorageProvider, get_storage_provider
from omniagent.app_server.web_client.default_web_client_config_injector import (
    DefaultWebClientConfigInjector,
)
from omniagent.app_server.web_client.web_client_config_injector import (
...
def get_openhands_provider_base_url() -> str | None:
    """Return the base URL for the OmniAgent provider, if configured.

    Falls back to LLM_BASE_URL for backward compatibility.
    """
    return os.getenv('OMNIAGENT_PROVIDER_BASE_URL') or os.getenv('LLM_BASE_URL') or None
...
# OmniAgent provider models use this proxy at the SDK transport boundary.
# Deployments (e.g. staging) may use a different LLM proxy, configured via
# OMNIAGENT_PROVIDER_BASE_URL.
_SDK_DEFAULT_PROXY = 'https://llm-proxy.app.github.com/ismailubts/OmniAgent/'
...
def resolve_provider_llm_base_url(
    ...
    When the model uses the public ``omniagent/`` prefix and the stored
    ...
    Args:
        model: LLM model name (e.g. ``omniagent/gpt-5.5``).
    ...
    """
    if not model or not model.startswith('omniagent/'):
        return base_url
...
def _get_default_lifespan():
    ...
    if 'saas' in (os.getenv('OMNIAGENT_CONFIG_CLS') or '').lower():
...
class AppServerConfig(OpenHandsModel):
    ...
    web_url: str | None = Field(
        ...
        description='The URL where OmniAgent is running (e.g., http://localhost:3000)',
    )
    ...
    openhands_provider_base_url: str | None = Field(
        ...
        description='Base URL for the OmniAgent provider',
    )
...
def config_from_env() -> AppServerConfig:
    # Import defaults...
    from omniagent.app_server.app_conversation.live_status_app_conversation_service import (  # noqa: E501
        LiveStatusAppConversationServiceInjector,
    )
    from omniagent.app_server.app_conversation.sql_app_conversation_info_service import (  # noqa: E501
        SQLAppConversationInfoServiceInjector,
    )
    from omniagent.app_server.app_conversation.sql_app_conversation_start_task_service import (  # noqa: E501
        SQLAppConversationStartTaskServiceInjector,
    )
    from omniagent.app_server.event.aws_event_service import (
        AwsEventServiceInjector,
    )
    from omniagent.app_server.event.filesystem_event_service import (
        FilesystemEventServiceInjector,
    )
    from omniagent.app_server.event.google_cloud_event_service import (
        GoogleCloudEventServiceInjector,
    )
    from omniagent.app_server.event_callback.sql_event_callback_service import (
        SQLEventCallbackServiceInjector,
    )
    from omniagent.app_server.sandbox.docker_sandbox_service import (
        DockerSandboxServiceInjector,
    )
    from omniagent.app_server.sandbox.docker_sandbox_spec_service import (
        DockerSandboxSpecServiceInjector,
    )
    from omniagent.app_server.sandbox.process_sandbox_service import (
        ProcessSandboxServiceInjector,
    )
    from omniagent.app_server.sandbox.process_sandbox_spec_service import (
        ProcessSandboxSpecServiceInjector,
    )
    from omniagent.app_server.sandbox.remote_sandbox_service import (
        RemoteSandboxServiceInjector,
    )
    from omniagent.app_server.sandbox.remote_sandbox_spec_service import (
        RemoteSandboxSpecServiceInjector,
    )
    from omniagent.app_server.user.auth_user_context import (
    ...
    if config.llm_model is None:
        from omniagent.app_server.config_api.default_llm_model_service import (
