...
from omniagent.app_server.sandbox.sandbox_models import SandboxInfo
...
from omniagent.app_server.utils.auth import looks_like_jwt
...
def get_project_dir(
    ...
    `.omniagent/` configuration (setup.sh, pre-commit.sh, skills/, etc.).
    ...
    - .omniagent/setup.sh execution
    - .omniagent/pre-commit.sh (git hooks setup)
    - .omniagent/skills/ (project skills)
    - PLAN.md path

    Args:
        working_dir: Base working directory path in the sandbox
            (e.g., '/workspace/project' from sandbox_spec)
        selected_repository: Repository name (e.g., 'OmniAgent/software-agent-sdk')
...
class AppConversationServiceBase(AppConversationService, ABC):
    ...
    async def load_and_merge_all_skills(
        ...
        - Public skills (from OmniAgent/skills GitHub repo)
        - User skills (from ~/.omniagent/skills/)
        - Organization skills (from {org}/.omniagent repo)
        - Project/repo skills (from repo .agents/skills/, .omniagent/microagents/, and legacy .omniagent/skills/)
    ...
    async def run_setup_scripts(
        ...
        # for all .omniagent/ features (setup.sh, pre-commit.sh, skills).
...
"""
    ...
    async def maybe_setup_git_hooks(
        ...
        """Set up git hooks if .omniagent/pre-commit.sh exists in the project root.

        Args:
            workspace: Remote workspace for command execution.
            project_dir: Project root directory (repo root when a repo is selected).
        """
        command = 'mkdir -p .git/hooks && chmod +x .omniagent/pre-commit.sh'
        ...
        with tempfile.TemporaryFile(mode='w+t') as temp_file:
            ...
            if download_result.success:
                ...
                if 'This hook was installed by OmniAgent' not in temp_file.read():
