...
from omniagent.app_server.integrations.service_types import ProviderType, SuggestedTask
...
# ('claude-code', 'codex', 'gemini-cli') is stored. Synced with omniagent.
...
class AppConversationInfo(BaseModel):
    ...
    def acp_server(self) -> str | None:
        ...
        A typed projection of the ``acpserver`` tag (same as omniagent) so
        ...
        ``agent_kind`` so a stray tag never reports a provider for an OmniAgent
        conversation.
        """
