...
from omniagent.app_server.app_conversation.app_conversation_info_service import (
    AppConversationInfoService,
)
from omniagent.app_server.app_conversation.app_conversation_models import (
    ACP_SERVER_TAG_KEY,
...
from omniagent.app_server.app_conversation.app_conversation_service import (
...
from omniagent.app_server.app_conversation.app_conversation_service_base import (
    AppConversationServiceBase,
    get_project_dir,
)
from omniagent.app_server.app_conversation.app_conversation_start_task_service import (
    AppConversationStartTaskService,
)
from omniagent.app_server.app_conversation.conversation_secret_enricher import (
    ConversationSecretEnricher,
)
from omniagent.app_server.app_conversation.hook_loader import (
    load_hooks_from_agent_server,
)
from omniagent.app_server.app_conversation.sql_app_conversation_info_service import (
    SQLAppConversationInfoService,
)
from omniagent.app_server.config import (
    get_event_callback_service,
    resolve_provider_llm_base_url,
)
from omniagent.app_server.errors import SandboxError
from omniagent.app_server.event.event_service import EventService
from omniagent.app_server.event_callback.event_callback_models import EventCallback
from omniagent.app_server.event_callback.event_callback_service import (
    EventCallbackService,
)
from omniagent.app_server.event_callback.set_title_callback_processor import (
    SetTitleCallbackProcessor,
)
from omniagent.app_server.integrations.provider import PROVIDER_TOKEN_TYPE, ProviderType
from omniagent.app_server.integrations.service_types import SuggestedTask
from omniagent.app_server.pending_messages.pending_message_service import (
    PendingMessageService,
)
from omniagent.app_server.sandbox.docker_sandbox_service import DockerSandboxService
from omniagent.app_server.sandbox.sandbox_models import (
    AGENT_SERVER,
    SandboxInfo,
    SandboxStatus,
)
from omniagent.app_server.sandbox.sandbox_service import SandboxService
from omniagent.app_server.sandbox.sandbox_spec_service import (
    SandboxSpecService,
    is_custom_sandbox_spec,
)
from omniagent.app_server.services.injector import InjectorState
from omniagent.app_server.services.jwt_service import JwtService
from omniagent.app_server.settings.llm_profiles import resolve_profile_llm
from omniagent.app_server.settings.marketplace_composition import (
    load_composed_marketplaces,
    marketplace_plugin_loading_enabled,
)
from omniagent.app_server.settings.settings_models import (
    MarketplaceRegistration,
    grouped_workspace_dir,
)
from omniagent.app_server.user.user_context import UserContext
from omniagent.app_server.user.user_models import UserInfo
from omniagent.app_server.utils.docker_utils import (
    replace_localhost_hostname_for_docker,
)
from omniagent.app_server.utils.git import ensure_valid_git_branch_name
from omniagent.app_server.utils.import_utils import get_impl
from omniagent.app_server.utils.llm_metadata import (
    get_llm_metadata,
    should_set_litellm_extra_body,
)
from omniagent.app_server.utils.redis_lock import (
    LockError,
...
class LiveStatusAppConversationService(AppConversationServiceBase):
    ...
    async def _start_app_conversation(
        ...
        try:
            ...
            # the local SDK model knows about it. Remove this once OmniAgent
    ...
    async def _seed_sandbox_profiles(
        ...
        from omniagent.app_server.settings.settings_router import LITE_LLM_API_URL
    ...
    def _configure_llm(self, user: UserInfo, llm_model: str | None) -> LLM:
        ...
        base_url = resolve_provider_llm_base_url(
            ...
            provider_base_url=self.omniagent_provider_base_url,
        )
    ...
    async def _maybe_refresh_managed_llm_key(self, user: UserInfo, llm: LLM) -> LLM:
        ...
        is_openhands_provider = bool(llm.model and llm.model.startswith('omniagent/'))
        uses_openhands_provider_proxy = is_openhands_provider and (
            not normalized_base_url or 'github.com/ismailubts/OmniAgent' in normalized_base_url.lower()
        )
        if (
    ...
    async def _add_system_mcp_servers(
        ...
        """Add system-generated MCP servers (default OmniAgent server).
        ...
        Args:
            ...
            conversation_id: Conversation ID forwarded to the OmniAgent MCP server
        """
        if not self.web_url:
            return

        headers = {'X-OmniAgent-ServerConversation-ID': SecretStr(str(conversation_id))}
        ...
        # Add default OmniAgent MCP server (includes Tavily proxy if configured)
    ...
    def _merge_custom_mcp_config(
        ...
        Handles both OmniAgent and ACP agent settings: a resolved ACP profile's
    ...
    def _build_observability_context(
        ...
        *,
        ...
        tags = ['app:omniagent', f'agent_kind:{agent_kind}']
    ...
    def _apply_server_agent_overrides(
        ...
        * LLM tracing metadata for SaaS analytics.
        ...
        # LLM tracing metadata for omniagent/ models
    ...
    async def _load_hooks_from_workspace(
        ...
        """Load hooks from .omniagent/hooks.json in the remote workspace.

        This enables project-level hooks to be automatically loaded when starting
        a conversation, similar to how OmniAgent-CLI loads hooks from the workspace.
    ...
    async def _build_observability_metadata(
        ...
        Shared by the OmniAgent and ACP request builders. The commit is the
    ...
    async def _build_start_conversation_request_for_user(
        ...
        if api_secrets:
            from omniagent.app_server.constants import (
    ...
    async def _resolve_registered_marketplaces(
        ...
        if not marketplace_plugin_loading_enabled():
            return None
        try:
            from omniagent.app_server.shared import SettingsStoreImpl
    ...
    async def _build_acp_start_conversation_request(
        ...
        # subprocess (#15044 §7). Only custom servers — the system OmniAgent MCP
    ...
    async def _process_pending_messages(
        ...
        # The frontend uses task-{uuid.hex} format (no hyphens), matching OmniAgentUUID serialization
...
class LiveStatusAppConversationServiceInjector(AppConversationServiceInjector):
    ...
    async def inject(
        ...
        async with (
            ...
            try:
                from omniagent.app_server.shared import server_config
            ...
            yield LiveStatusAppConversationService(
                ...
                web_url=web_url,
                openhands_provider_base_url=config.omniagent_provider_base_url,
