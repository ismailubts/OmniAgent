...
from omniagent.app_server.integrations.provider import ProviderHandler, ProviderType
...
async def _determine_org_repo_path(
    ...
    Examples:
        - GitHub/Bitbucket: ('owner/.omniagent', 'owner')
        - GitLab: ('owner/omniagent-config', 'owner')
        - Azure DevOps: ('org/omniagent-config/omniagent-config', 'org')
    """
    ...
    if is_gitlab:
        org_openhands_repo = f'{org_name}/omniagent-config'
    elif is_azure_devops:
        org_openhands_repo = f'{org_name}/omniagent-config/omniagent-config'
    else:
        org_openhands_repo = f'{org_name}/.omniagent'
...
def _candidate_repo_paths(provider: ProviderType, owner: str) -> list[str]:
    ...
    GitHub-style providers expose two independent repos (``.omniagent`` and
    ``.agents``) that are loaded concurrently. GitLab and Azure DevOps use a
    single ``omniagent-config`` repository (they have no ``.agents`` analog).
    ...
    Returns:
        List of repository paths (e.g., ['owner/.omniagent', 'owner/.agents']).
    """
    if provider == ProviderType.GITLAB:
        return [f'{owner}/omniagent-config']
    if provider == ProviderType.AZURE_DEVOPS:
        return [f'{owner}/omniagent-config/omniagent-config']
    return [f'{owner}/.omniagent', f'{owner}/.agents']
...
async def build_org_configs(
    ...
    resolving both ``.omniagent`` and ``.agents`` repos for GitHub-style
