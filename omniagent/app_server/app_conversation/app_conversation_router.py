"""Sandboxed Conversation router for OmniAgent App Server."""
...
from omniagent.analytics import get_analytics_service, resolve_analytics_context
...
from omniagent.app_server.sandbox.sandbox_spec_service import SandboxSpecService
from omniagent.app_server.services.db_session_injector import set_db_session_keep_open
from omniagent.app_server.services.httpx_client_injector import (
...
from omniagent.app_server.settings.settings_models import Settings
from omniagent.app_server.settings.settings_router import LITE_LLM_API_URL
from omniagent.app_server.user.specifiy_user_context import USER_CONTEXT_ATTR
from omniagent.app_server.user.user_context import UserContext
from omniagent.app_server.user_auth import get_user_settings
from omniagent.app_server.utils.dependencies import get_dependencies
...
async def _get_agent_server_context(
    ...
    if not sandbox_spec:
        ...
        # sandbox spec versions when updating OmniAgent. When the SandboxSpecServices
...
async def get_conversation_skills(
    ...
    - Global skills (OmniAgent/skills/)
    - User skills (~/.omniagent/skills/)
    - Organization skills (org/.omniagent repository)
    - Repository skills (repo .agents/skills/, .omniagent/microagents/, and legacy .omniagent/skills/)
...
async def get_conversation_hooks(
    ...
    workspace (i.e. `{project_dir}/.omniagent/hooks.json`) at request time.

    Note:
        This is intentionally a "live" view of the workspace configuration.
        If `.omniagent/hooks.json` changes over time, this endpoint reflects the
    ...
    try:
        ...
        try:
            ...
            if analytics and user_id:
                from omniagent.analytics.analytics_context import AnalyticsContext
