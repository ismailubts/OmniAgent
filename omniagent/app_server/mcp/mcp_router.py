...
from omniagent.app_server.integrations.azure_devops.azure_devops_service import (
    AzureDevOpsServiceImpl,
)
from omniagent.app_server.integrations.bitbucket.bitbucket_service import (
    BitBucketServiceImpl,
)
from omniagent.app_server.integrations.bitbucket_data_center.bitbucket_dc_service import (
    BitbucketDCServiceImpl,
)
from omniagent.app_server.integrations.github.github_service import GithubServiceImpl
from omniagent.app_server.integrations.gitlab.gitlab_service import GitLabServiceImpl
from omniagent.app_server.integrations.provider import ProviderToken
from omniagent.app_server.integrations.service_types import GitService, ProviderType
...
from omniagent.app_server.user_auth import (
    get_access_token,
    get_provider_tokens,
    get_user_id,
)
...
HOST = f'https://{os.getenv("WEB_HOST", "app.github.com/ismailubts/OmniAgent").strip()}'
...
async def get_conversation_link(
    ...
    """Appends a followup link, in the PR body, to the OmniAgent conversation that opened the PR"""
...
async def create_pr(
    ...
    """Open a PR in GitHub"""
    logger.info('Calling OmniAgent MCP create_pr')

    request = get_http_request()
    headers = request.headers
    conversation_id = headers.get('X-OmniAgent-ServerConversation-ID', None)
...
async def create_mr(
    ...
    """Open a MR in GitLab"""
    logger.info('Calling OmniAgent MCP create_mr')
...
async def create_bitbucket_pr(
    ...
    logger.info('Calling OmniAgent MCP create_bitbucket_pr')
...
async def create_bitbucket_data_center_pr(
    ...
    logger.info('Calling OmniAgent MCP create_bitbucket_data_center_pr')
...
async def create_azure_devops_pr(
    ...
    logger.info('Calling OmniAgent MCP create_azure_devops_pr')
