...
from omniagent.app_server.config import depends_user_context
...
GLOBAL_SKILLS_DIR = Path(omniagent.__file__).parent.parent / 'skills'
USER_SKILLS_DIR = Path.home() / '.omniagent' / 'microagents'
...
async def _clone_marketplace_repo(
    ...
    try:
        clone_dir = Path(
            tempfile.mkdtemp(prefix=f'omniagent_marketplace_{marketplace.name}_')
        )
