# TODO: Merge this module with omniagent.app_server.config
import os

from omniagent.app_server.types import AppMode, ServerConfigInterface
...
class ServerConfig(ServerConfigInterface):
    config_cls = os.environ.get('OMNIAGENT_CONFIG_CLS', None)
    ...
    settings_store_class: str = (
        'omniagent.app_server.settings.file_settings_store.FileSettingsStore'
    )
    secret_store_class: str = (
        'omniagent.app_server.secrets.file_secrets_store.FileSecretsStore'
    )
    user_auth_class: str = (
        'omniagent.app_server.user_auth.default_user_auth.DefaultUserAuth'
    )
