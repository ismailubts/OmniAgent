...
from omniagent.app_server.sandbox.docker_sandbox_spec_service import get_docker_client
...
class DockerSandboxService(SandboxService):
    ...
    async def start_sandbox(
        ...
        # frontend when running OmniAgent on a remote machine.
...
class DockerSandboxServiceInjector(SandboxServiceInjector):
    ...
    host_port: int = Field(
        default=3000,
        description=(
            'The port on which the main OmniAgent app server is running. '
            'Used for webhook callbacks from agent-server containers. '
            'If running OmniAgent on a non-default port, set this to match. '
