...
#
# Self-hosted mode builds the ``omniagent/…`` model list from the SDK's
# ``VERIFIED_OMNIAGENT_MODELS``.  SaaS mode overrides it with the database
...
# Build the managed-provider model list from the SDK (openhands/ prefix required by LiteLLM).
OMNIAGENT_MODELS: list[str] = [f'openhands/{m}' for m in _SDK_OPENHANDS]
...
DEFAULT_OMNIAGENT_MODEL = 'openhands/minimax-m2.7'
...
class ModelsResponse(BaseModel):
    ...
    * ``verified_models`` — model names (without provider prefix) that
      OmniAgent has verified to work well.
...
def is_openhands_model(model: str | None) -> bool:
    """Return True when the model uses the public OmniAgent provider prefix."""
    return bool(model and model.startswith('omniagent/'))
...
def resolve_llm_base_url(
    ...
    * ``omniagent/app_server/settings/settings_router._post_merge_llm_fixups``
    ...
    * ``base_url is None`` + known OmniAgent / managed model → ``managed_proxy_url``.
...
def get_openhands_models(
    ...
    """Return the list of OmniAgent-provider model strings.

    In self-hosted mode *verified_models* is ``None`` (or empty) and the
    hardcoded ``OMNIAGENT_MODELS`` list is used.  In SaaS mode the caller
    passes the database-backed list which takes precedence.

    Args:
        verified_models: Optional list of ``"omniagent/<name>"`` strings
            loaded from the verified-models database table.

    Returns:
        A list such as ``["openhands/claude-opus-4-6", ...]``.
    """
    return verified_models if verified_models else OMNIAGENT_MODELS
...
def _derive_verified_models(openhands_models: list[str]) -> list[str]:
    """Extract the bare model names from the ``omniagent/…`` model list."""
    return [
        m.removeprefix('omniagent/')
        for m in openhands_models
        if m.startswith('omniagent/')
    ]
...
def get_supported_llm_models(
    ...
    * a list of *verified* model names (the OmniAgent-curated subset),
    ...
    * the recommended default model.
    ...
    return ModelsResponse(
        ...
        default_model=DEFAULT_OMNIAGENT_MODEL,
    )
