"""Utility functions for LLM metadata in OmniAgent V1 conversations."""
...
def should_set_litellm_extra_body(model_name: str) -> bool:
    ...
    Only set litellm_extra_body for managed OmniAgent proxy models (``openhands/`` prefix
    from the SDK) to avoid issues with providers that don't support extra_body parameters.
...
def get_llm_metadata(
    ...
    """Generate LLM metadata for OmniAgent V1 conversations.
    ...
    """
    omniagent_version = omniagent.__version__

    metadata: dict[str, Any] = {
        'trace_version': omniagent_version,
        'tags': [
            'app:omniagent',
            f'model:{model_name}',
            f'type:{llm_type}',
            f'web_host:{os.environ.get("WEB_HOST", "unspecified")}',
            f'omniagent_version:{omniagent_version}',
