...
def log_uncaught_exceptions(
    ...
    omniagent_logger.error('Uncaught exception', exc_info=(ex_cls, ex, tb))


sys.excepthook = log_uncaught_exceptions
omniagent_logger = logging.getLogger('omniagent')
current_log_level = logging.INFO

if LOG_LEVEL in logging.getLevelNamesMapping():
    current_log_level = logging.getLevelNamesMapping()[LOG_LEVEL]
omniagent_logger.setLevel(current_log_level)

if DEBUG:
    omniagent_logger.addFilter(StackInfoFilter())

if current_log_level == logging.DEBUG:
    omniagent_logger.debug('DEBUG mode enabled.')

_sensitive_data_filter = SensitiveDataFilter(omniagent_logger.name)

if LOG_JSON:
    _json_handler = json_log_handler(current_log_level)
    _json_handler.addFilter(_sensitive_data_filter)
    omniagent_logger.addHandler(_json_handler)
    ...
    omniagent_logger.addHandler(_console_handler)

# Keep the filter on the logger for records logged directly to omniagent_logger,
# and on the handlers for records propagated from child loggers (e.g.
# omniagent.app_server.*) — Python only checks the originating logger's
# filters, not ancestor filters, during propagation.
omniagent_logger.addFilter(_sensitive_data_filter)
omniagent_logger.propagate = False
omniagent_logger.debug('Logging initialized')

LOG_DIR = os.path.join(
    # parent dir of omniagent/app_server/utils (i.e., root of the repo)
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    'logs',
)

if LOG_TO_FILE:
    omniagent_logger.addHandler(
        get_file_handler(LOG_DIR, current_log_level)
    )  # default log to project root
    omniagent_logger.debug(f'Logging to file in: {LOG_DIR}')
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        super().emit(record)
        self.stream.close()
        omniagent_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
...
class OmniAgentLoggerAdapter(logging.LoggerAdapter):
    extra: dict

    def __init__(
        self, logger: logging.Logger = omniagent_logger, extra: dict | None = None
    ) -> None:
        self.logger = logger
        self.extra = extra or {}
...
def _uvicorn_json_log_config() -> dict:
    return {
        ...
        'filters': {
            'redact_url_params': {
                '()': 'omniagent.app_server.utils.logger.RedactURLParamsFilter',
            },
        },
        'formatters': {
