# OmniAgent FileStore Module

The file store module provides different storage backends for file operations in OmniAgent. This module implements a common interface (`FileStore`) that allows for interchangeable storage backends.
...
```python
from omniagent.app_server.file_store import get_file_store, LocalFileStore
...
To configure the file store in OmniAgent, use the following configuration options:
