...
describe("useAppTitle", () => {
  ...
  it("should return 'OmniAgent' if is OSS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() => expect(result.current).toBe("OmniAgent"));
  });

  it("should return 'OmniAgent Cloud' if is SaaS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    getConfigSpy.mockResolvedValue({
      app_mode: "saas",
    });
    ...
    await waitFor(() => expect(result.current).toBe("OmniAgent Cloud"));
  });

  it("should return '{some title} | OmniAgent' if is OSS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() =>
      expect(result.current).toBe("My Conversation | OmniAgent"),
    );
  });

  it("should return '{some title} | OmniAgent Cloud' if is SaaS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() =>
      expect(result.current).toBe(
        "Another Conversation Title | OmniAgent Cloud",
      ),
    );
  });
