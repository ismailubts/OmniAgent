...
import type { OmniAgentParsedEvent } from "../core/index";
 ...
 */
export const isMessageEvent = (event: OmniAgentEvent): event is MessageEvent =>
 ...
 */
export const isActionEvent = (event: OmniAgentEvent): event is ActionEvent =>
  event.source === "agent" &&
  "action" in event &&
  event.action !== null &&
 ...
 */
export const isDisplayableErrorEvent = (event: OmniAgentEvent): boolean =>
...
/**
 * TEMPORARY: Type guard to check if an event is a V1 OmniAgentEvent
...
export function isV1Event(
  event: OmniAgentEvent | OmniAgentParsedEvent,
): event is OmniAgentEvent {
...
/**
 * TEMPORARY: Type guard to check if an event is a V0 OmniAgentParsedEvent
 *
 * @deprecated This is temporary until full V1 migration is complete
 */
