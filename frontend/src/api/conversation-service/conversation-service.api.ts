...
class ConversationService {
  ...
  static async getVSCodeUrl(
    ...
    const { data } = await omniAgentApi.get<GetVSCodeUrlResponse>(url, {
  ...
  static async getTrajectory(
    ...
    const { data } = await omniAgentApi.get<GetTrajectoryResponse>(url, {
      headers: this.getConversationHeaders(),
    });
    return data;
  }
  ...
  static async uploadFiles(
    ...
    const response = await omniAgentApi.post<FileUploadSuccessResponse>(
      url,
      formData,
      {
        headers: {
