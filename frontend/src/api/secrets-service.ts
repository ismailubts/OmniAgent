...
export class SecretsService {
  ...
  static async searchSecrets(
    ...
    const { data } = await omniAgentApi.get<CustomSecretPage>(url);
    return data;
  }
  ...
  static async createSecret(name: string, value: string, description?: string) {
    ...
    const { status } = await omniAgentApi.post("/api/v1/secrets", secret);
    return status === 201;
  }
  ...
  static async updateSecret(id: string, name: string, description?: string) {
    ...
    const { status } = await omniAgentApi.put(`/api/v1/secrets/${id}`, secret);
    return status === 200;
  }

  static async deleteSecret(id: string) {
    const { status } = await omniAgentApi.delete<boolean>(`/api/v1/secrets/${id}`);
    return status === 200;
  }
  ...
  static async addGitProvider(providers: Record<Provider, ProviderToken>) {
    ...
    const { data } = await omniAgentApi.post<boolean>(
  ...
  static async deleteGitProviders() {
    const { status } = await omniAgentApi.delete("/api/v1/secrets/git-providers");
    return status === 200;
  }
}
