...
class SettingsService {
  ...
  static async getSettings(): Promise<Settings> {
    const { data } = await omniAgentApi.get<Settings>("/api/v1/settings");
    return data;
  }
  ...
  static async getSettingsSchema(): Promise<SettingsSchema> {
    const { data } = await omniAgentApi.get<SettingsSchema>(
  ...
  static async saveSettings(
    ...
    const response = await omniAgentApi.post("/api/v1/settings", settings);
