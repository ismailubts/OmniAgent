...
import { GitChange, GitChangeDiff } from "../omniagent-api.types";
...
class GitService {
  ...
  static async searchGitRepositories(
    ...
    const { data } = await omniAgentApi.get<RepositoryPage>(
  ...
  static async getRepositoryBranches(
    ...
    const { data } = await omniAgentApi.get<BranchPage>(
  ...
  static async getUserInstallations(
    ...
    const { data } = await omniAgentApi.get<InstallationPage>(
  ...
  static async getGitChanges(conversationId: string): Promise<GitChange[]> {
    ...
    const { data } = await omniAgentApi.get<GitChange[]>(url, {
  ...
  static async getGitChangeDiff(
    ...
    const { data } = await omniAgentApi.get<GitChangeDiff>(url, {
      params: { path },
      headers: ConversationService.getConversationHeaders(),
    });
    return data;
