...
Services are the abstraction layer between frontend components and backend APIs. They encapsulate HTTP requests using the shared `omniAgentApi` axios instance and provide typed methods for each endpoint.
...
└── omniagent-axios.ts           # Shared axios instance
```
...
export const featureService = {
  getFeature: async ({ id }: { id: string }) => {
    const { data } = await omniAgentApi.get<Feature>(`/api/features/${id}`);
    return data;
  },

  createFeature: async ({ name, description }: CreateFeatureParams) => {
    const { data } = await omniAgentApi.post<Feature>("/api/features", {
      name,
      description,
    });
    return data;
