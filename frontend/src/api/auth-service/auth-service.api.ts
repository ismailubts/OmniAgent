...
class AuthService {
  ...
  static async authenticate(
    ...
    await omniAgentApi.post<AuthenticateResponse>("/api/authenticate");
    return true;
  }
  ...
  static async getGitHubAccessToken(
    ...
    const { data } = await omniAgentApi.post<GitHubAccessTokenResponse>(
  ...
  static async logout(appMode: WebClientConfig["app_mode"]): Promise<void> {
    ...
    await omniAgentApi.post(endpoint);
  }
}
