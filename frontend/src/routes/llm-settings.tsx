...
const isProviderDefaultBaseUrl = (model: string, baseUrl: string) => {
  // For omniagent/* models, base_url is server-owned (auto-filled on save) —
  // never treat it as user customization that warrants the advanced view.
  // Accepted edge case: a deliberately customized base_url on an omniagent/*
  // model also lands on basic; values are preserved, Advanced is a click away.
  if (model.startsWith("omniagent/")) {
    return true;
  }
...
export function LlmSettingsScreen({
    ...
    ({ values, isDisabled, view, onChange }: SdkSectionHeaderProps) => {
      ...
      const activeProvider =
        view === "basic"
          ...
          : derivedProvider;
      const shouldUseOmniAgentKey =
        isSaasMode && activeProvider === "openhands";
      // The OmniAgent key help links to OmniAgent Cloud keys/pricing — only
      // meaningful on enterprise cloud, not self-hosted managed installs.
      const showOmniAgentApiKeyHelp =
        isEnterpriseCloud &&
        modelValue.startsWith("omniagent/") &&
        allowUserLlmConfiguration;
        ...
        <div className="flex flex-col gap-6">
          ...
          {view === "basic" || !allowUserLlmConfiguration ? (
            ...
            >
              ...
              {showOmniAgentApiKeyHelp ? (
                <OmniAgentApiKeyHelp testId="omniagent-api-key-help" />
              ) : null}
            ...
            >
              ...
              {showOmniAgentApiKeyHelp ? (
                <OmniAgentApiKeyHelp testId="omniagent-api-key-help-2" />
              ) : null}
  ...
  const buildPayload = React.useCallback(
    (
      ...
      if (shouldUseOmniAgentKey && llm.model !== undefined) {
        llm.api_key = "";
      ...
      if (profileFormMode === "edit") {
        ...
        if (shouldUseOmniAgentKey && llm.api_key === undefined) {
          llm.api_key = "";
        }
        agentSettings.llm = llm;
      }
      ...
      // OmniAgent-managed saves force api_key to "" above — that's not a
      ...
      lastSavedApiKeyTypedRef.current =
        !shouldUseOmniAgentKey && typedApiKey.length > 0;
