...
export const getRepoMdCreatePrompt = (
  ...
  return `Please explore this repository. Create the file .omniagent/microagents/repo.md with:
            ${
              query
                ? `- ${query}`
...
/**
 * Get the OmniAgent query string based on the provider
 * @param provider The git provider
 * @returns The query string for searching OmniAgent repositories
 */
export const getOmniAgentQuery = (provider: Provider | null): string => {
  const providerRepositorySuffix: Record<string, string> = {
    gitlab: "omniagent-config",
    azure_devops: "omniagent-config",
    default: ".omniagent",
  } as const;
...
/**
 * Check if a repository has the OmniAgent suffix based on the provider
 * @param repo The Git repository to check
 * @param provider The git provider
 * @returns True if the repository has the OmniAgent suffix
 */
export const hasOmniAgentSuffix = (
  repo: GitRepository,
  provider: Provider | null,
): boolean => repo.full_name.endsWith(`/${getOmniAgentQuery(provider)}`);
