import { MessageEvent, OmniAgentEvent } from "#/types/v1/core";
...
const findLastUserMessageIndex = (events: OmniAgentEvent[]): number => {
...
const getFinalAgentText = (event: OmniAgentEvent): string | null => {
...
const finalizeStreamingDeltasInPlace = (
  finalEvent: OmniAgentEvent,
  uiEvents: OmniAgentEvent[],
): OmniAgentEvent[] | null => {
  ...
  // streamed text (mirrors omniagent #1552).
...
const supersedeStreamingDeltasForAction = (
  ...
  // instead of dropping it (mirrors omniagent #1552).
  ...
  const nextUiEvents: OmniAgentEvent[] = [];
