...
export const generateAuthUrl = (
  ...
  if (authUrl) {
    ...
    finalAuthUrl = requestUrl.hostname
      .replace(/(^|\.)staging\.omniagent\.dev$/, "$1auth.staging.omniagent.dev")
      .replace(/(^|\.)app\.omniagent\.dev$/, "auth.app.omniagent.dev")
      .replace(/(^|\.)localhost$/, "auth.localhost");
  ...
  return `${finalAuthUrl}/realms/omniagent/protocol/openid-connect/auth?client_id=omniagent&kc_idp_hint=${identityProvider}&response_type=code&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${encodeURIComponent(scope)}&state=${encodeURIComponent(state)}`;
};
