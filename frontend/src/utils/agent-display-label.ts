...
import type { AgentKind } from "#/api/omniagent-api.types";
 ...
 * Synced with omniagent and the OmniAgent backend. Constrained to ^[a-z0-9]+$
 ...
 * (OmniAgent logo, Claude/OpenAI/Gemini mark), and the text is the model label.
 * ACP models use the curated label from the provider registry (falling back to
 * the raw id); native OmniAgent models use ``formatLlmModel``. For ACP
