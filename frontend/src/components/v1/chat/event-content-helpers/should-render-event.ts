import { OmniAgentParsedEvent } from "#/types/core";
import { OmniAgentEvent } from "#/types/v1/core";
import {
  isActionEvent,
  isObservationEvent,
  isMessageEvent,
...
export const shouldRenderEvent = (event: OmniAgentEvent) => {
...
export const hasUserEvent = (events: OmniAgentEvent[]) =>
...
export const getRenderedV1Events = (
  events: ReadonlyArray<OmniAgentEvent | OmniAgentParsedEvent>,
): OmniAgentEvent[] => events.filter(isV1Event).filter(shouldRenderEvent);
