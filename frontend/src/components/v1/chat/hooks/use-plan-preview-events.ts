 ...
 */
function groupEventsByPhase(events: OmniAgentEvent[]): OmniAgentEvent[][] {
  const phases: OmniAgentEvent[][] = [];
  let currentPhase: OmniAgentEvent[] = [];
 ...
 */
export function usePlanPreviewEvents(allEvents: OmniAgentEvent[]): Set<string> {
  return useMemo(() => {
