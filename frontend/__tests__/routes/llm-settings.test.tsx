...
async function selectProvider(providerLabel: "OmniAgent" | "OpenAI") {
...
describe("LlmSettingsScreen", () => {
  ...
  it("defaults to basic view when an OmniAgent managed model has no base URL", async () => {
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
  ...
  it("opens basic view when an OmniAgent managed model has a server-filled base URL", async () => {
    // Managed omniagent/* profiles always come back with a base_url — the
  ...
  it("opens basic view when editing an org-scope OmniAgent managed profile with a server-filled base URL", async () => {
  ...
  it("opens basic view even when an OmniAgent model has a non-managed base URL", async () => {
    // Accepted edge case: the server owns base_url for omniagent/* models,
  ...
  it("treats a litellm_proxy model with the managed proxy URL as an explicit custom endpoint", async () => {
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
      buildSettingsWithAdvancedToggle({
        ...
        llm_base_url: "https://llm-proxy.app.github.com/ismailubts/OmniAgent",
        agent_settings: {
          llm: {
    ...
    expect(
      screen.queryByTestId("omniagent-api-key-help-2"),
    ).not.toBeInTheDocument();
  });
  ...
  it("uses the docs.omniagent.dev domain for the API key help link", async () => {
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
      buildSettings({
        llm_model: "openai/gpt-4o",
        agent_settings: {
    ...
    expect(helpLink.querySelector("a")).toHaveAttribute(
      "href",
      "https://github.com/ismailubts/OmniAgent#quickstart",
    );
  });
  ...
  it("hides the API key input for OmniAgent provider on enterprise cloud", async () => {
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(buildSettings());
    ...
    expect(screen.getByTestId("omniagent-api-key-help")).toBeInTheDocument();
  });

  it("hides the OmniAgent API key help on self-hosted OHE", async () => {
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(buildSettings());
    ...
    await screen.findByTestId("llm-settings-screen");
    expect(
      screen.queryByTestId("omniagent-api-key-help"),
    ).not.toBeInTheDocument();
  });

  it("shows the API key input for non-OmniAgent providers in SaaS mode", async () => {
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
      buildSettings({
        llm_model: "openai/gpt-4o",
  ...
  it("keeps an existing custom base URL when saving basic view without a model change", async () => {
    // Use a non-managed model: omniagent/* models treat any base_url as
  ...
  it("does not reveal all-only fields after save when refetch returns an OmniAgent managed model", async () => {
    const schema = structuredClone(
      MOCK_DEFAULT_USER_SETTINGS.agent_settings_schema!,
    );
    const llmSection = schema.sections.find((section) => section.key === "llm");
  ...
  describe("API key visibility in Basic Settings", () => {
    it("should hide API key input when SaaS mode is enabled and OmniAgent provider is selected", async () => {
      vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
        buildSettings(),
      );
      ...
      await waitFor(() => {
        expect(providerInput).toHaveValue("OmniAgent");
      });
    ...
    it("should show API key input when SaaS mode is enabled and non-OmniAgent provider is selected", async () => {
      vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
        buildSettings({
          llm_model: "openai/gpt-4o",
          agent_settings: { llm: { model: "openai/gpt-4o" } },
    ...
    it("should show API key input when OSS mode is enabled and OmniAgent provider is selected", async () => {
      vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
        buildSettings(),
      );
    ...
    it("should show API key input when OSS mode is enabled and non-OmniAgent provider is selected", async () => {
      vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
        buildSettings({
          llm_model: "openai/gpt-4o",
          agent_settings: { llm: { model: "openai/gpt-4o" } },
    ...
    it("should hide API key input when switching from non-OmniAgent to OmniAgent provider in SaaS mode", async () => {
      vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
        buildSettings({
          llm_model: "openai/gpt-4o",
          agent_settings: { llm: { model: "openai/gpt-4o" } },
      ...
      await selectProvider("OmniAgent");
    ...
    it("should show API key input when switching from OmniAgent to non-OmniAgent provider in SaaS mode", async () => {
      vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
        buildSettings(),
      );
  ...
  describe("Add Profile (create flow)", () => {
    ...
    it("does not preselect the active provider when creating a profile in SaaS mode", async () => {
      // Default settings use an OmniAgent model; previously the create form
    ...
    it("only shows the API key input once a key-taking provider is selected", async () => {
      ...
      // The managed OmniAgent provider keeps it hidden in SaaS mode (keys
    ...
    it("hydrates the edit form with the canonical model when the profile stores a hidden alias", async () => {
      // Legacy profile model "omniagent/custom-llm" is a hidden alias that
