...
describe("AgentSettingsScreen — minimal generic ACP UX", () => {
  ...
  it("clears acp_* fields when switching back to OmniAgent", async () => {
    vi.spyOn(OptionService, "getConfig").mockResolvedValue(baseConfig);
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue({
      ...MOCK_DEFAULT_USER_SETTINGS,
      agent_settings: {
    ...
    const ohOption = await screen.findByRole("option", {
      name: "SETTINGS$AGENT_TYPE_OMNIAGENT",
    });
  ...
  const omniAgentApiSettingsWithConcurrency = (toolConcurrencyLimit: number) => ({
    ...MOCK_DEFAULT_USER_SETTINGS,
    agent_settings: {
  ...
  it("saves a coerced tool_concurrency_limit on the OmniAgent path", async () => {
    vi.spyOn(OptionService, "getConfig").mockResolvedValue(baseConfig);
    vi.spyOn(SettingsService, "getSettings").mockResolvedValue(
      omniAgentApiSettingsWithConcurrency(1),
    );
    const saveSpy = vi
      .spyOn(SettingsService, "saveSettings")
      .mockResolvedValue(true);
