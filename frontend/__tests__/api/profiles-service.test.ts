...
describe("ProfilesService", () => {
  ...
  describe("listProfiles", () => {
    it("GETs /api/v1/settings/profiles and returns the body", async () => {
      ...
      vi.mocked(omniAgentApi.get).mockResolvedValue({ data: body });

      await expect(ProfilesService.listProfiles()).resolves.toEqual(body);
      expect(omniAgentApi.get).toHaveBeenCalledWith("/api/v1/settings/profiles");
    });
  });

  describe("saveProfile", () => {
    it("POSTs the name-encoded URL with the provided request body", async () => {
      vi.mocked(omniAgentApi.post).mockResolvedValue({ data: {} });
      ...
      expect(omniAgentApi.post).toHaveBeenCalledWith(
  ...
  describe("deleteProfile", () => {
    it("DELETEs the name-encoded URL", async () => {
      vi.mocked(omniAgentApi.delete).mockResolvedValue({ data: {} });

      await ProfilesService.deleteProfile("openai_gpt-4o");

      expect(omniAgentApi.delete).toHaveBeenCalledWith(
        "/api/v1/settings/profiles/openai_gpt-4o",
      );
    });
  });
  ...
  describe("error propagation", () => {
    ...
    it("rejects when listProfiles fails", async () => {
      ...
      vi.mocked(omniAgentApi.get).mockRejectedValue(err);
    ...
    it("rejects when saveProfile fails", async () => {
      vi.mocked(omniAgentApi.post).mockRejectedValue(new Error("409 conflict"));
    ...
    it("rejects when deleteProfile fails", async () => {
      vi.mocked(omniAgentApi.delete).mockRejectedValue(new Error("boom"));
      await expect(ProfilesService.deleteProfile("x")).rejects.toThrow("boom");
    });

    it("rejects when activateProfile fails", async () => {
      vi.mocked(omniAgentApi.post).mockRejectedValue(new Error("not found"));
    ...
    it("rejects when renameProfile fails", async () => {
      vi.mocked(omniAgentApi.post).mockRejectedValue(new Error("already exists"));
