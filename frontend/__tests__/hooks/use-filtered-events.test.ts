...
import type { OmniAgentAction } from "#/types/core/actions";
import type { ActionEvent, MessageEvent } from "#/types/v1/core";
import { SecurityRisk } from "#/types/v1/core";

// --- V0 event factories ---

function createV0UserMessage(id: number): OmniAgentAction {
  return {
    id,
    source: "user",
    action: "message",
...
function createV0AgentMessage(id: number): OmniAgentAction {
  return {
    id,
    source: "agent",
    action: "message",
...
function createV0SystemEvent(id: number): OmniAgentAction {
  return {
    id,
    source: "environment",
    action: "system",
    args: {
      content: "source .omniagent/setup.sh",
      tools: null,
      omniagent_version: null,
      agent_class: null,
    },
