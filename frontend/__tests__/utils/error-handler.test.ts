...
describe("Error Handler", () => {
  ...
  describe("isBudgetOrCreditError", () => {
    it("identifies OmniAgent budget and credit limit errors", () => {
      expect(
        isBudgetOrCreditError(
          "Budget has been exceeded! Current cost: 18.51, Max budget: 18.24",
        ),
      ).toBe(true);
      expect(isBudgetOrCreditError("OmniAgent Credits are exhausted")).toBe(
        true,
      );
      expect(isBudgetOrCreditError("Credit limit reached")).toBe(true);
    });

    it("does not rewrite provider-side credit messages as OmniAgent billing errors", () => {
      expect(
        isBudgetOrCreditError(
