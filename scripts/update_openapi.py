#!/usr/bin/env python3
"""
Update OmniAgent OpenAPI documentation.
...
Behavior:
- Uses omniagent.app_server.app.app.openapi() to build the spec.
...
os.environ['OMNIAGENT_LOG_LEVEL'] = 'CRITICAL'
...
try:
    ...
    print(f'Error importing OmniAgent modules: {e}')
    print(
...
def update_openapi_spec(spec_path, backup=True):
    ...
    if 'servers' in current_spec:
        ...
        new_spec['servers'] = [
            {'url': 'https://github.com/ismailubts/OmniAgent', 'description': 'Production server'},
